...
# # Genetics
# if(isVirtualClass(class(s@gene)) || is.logical(s@gene)) {
#   genetics <<- F
# } else {
#   genetics <<- T
#   gen <<- s@gene  # shorthand
#  
#   # Neutral genetics
#   if(!is.logical(gen@Traits@Neutral)) neutral_genetics <<- T
#  
#   # Genetic load
#   if(!is.logical(gen@Traits@GeneticLoad)) geneticload <<- T
#  
#   # Dispersal-related genes
#   if(!is.logical(gen@Traits@EmigrationGenes)) emigration_genes <<- T
#   if(!is.logical(gen@Traits@SettlementGenes)) settlement_genes <<- T
#   if(!is.logical(gen@Traits@CorrRWGenes) ||
#      !is.logical(gen@Traits@SMSGenes) ||
#      !is.logical(gen@Traits@KernelGenes)) transfer_genes <<- T
# }
...
# help with empty character fields
safe_char <- function(x, default = "-9") {
  if(length(x) == 0 || is.null(x)) return(default)
  as.character(x)
}
# # help function for genetics
#   fmt <<- function(x) {
#     if(is.null(x)) return("NULL")
#     if(is.logical(x)) return(as.character(x))
#     if(is.matrix(x)) return(paste(apply(x, 1, paste, collapse=" "), collapse=" | "))
#     paste(x, collapse=", ")
#   }
...
if(dispersal_kernel){
  if(class(s@dispersal@Transfer@Distances)[1]=="matrix"){
...
if(is.matrix(s@dispersal@Settlement@MinSteps)) {
    Min_steps <- array_to_LaTeX(s@dispersal@Settlement@MinSteps)
} else {
    Min_steps <- toString(as.vector(s@dispersal@Settlement@MinSteps))
}

if(is.matrix(s@dispersal@Settlement@MaxSteps)) {
    Max_steps <- array_to_LaTeX(s@dispersal@Settlement@MaxSteps)
} else {
    Max_steps <- toString(as.vector(s@dispersal@Settlement@MaxSteps))
}

if(is.matrix(s@dispersal@Settlement@MaxStepsYear)) {
    Max_steps_year <- array_to_LaTeX(s@dispersal@Settlement@MaxStepsYear)
} else {
    Max_steps_year <- toString(as.vector(s@dispersal@Settlement@MaxStepsYear))
}
# Management
...
if(real_land){
  Landscape <- data.frame(
    ...
    "Values"=c(
      ...
      CostsFile,
      DynamicLandYears,
      if(matrix_input) "provided as matrix" else safe_char(s@land@SpDistFile),
      s@land@SpDistResolution,
      if(spatiotemporal_demography && matrix_input) "provided as matrices",
      if(spatiotemporal_demography && !matrix_input) demogScaleFile,
...
Dispersal <- data.frame(
  ...
  "Description                        "=c(
    ...
    if(dispersal_kernel){"Settlement codes (for DispersalKernel) for all stages/sexes.0 = die (default) 1 = wait (stage-structured models only) 2 = randomly choose a suitable neighbouring cell or die 3 = randomly choose a suitable neighbouring cell or wait (stage-structured models only)"}
      else {"Settlement probability parameters for all stages/sexes."},
...
#   # gen <- s@gene
#  
#   # ---- GENETICS (top-level GeneticsParams) ----
#   Genetics <<- data.frame(
#     "Parameter" = c(
#       "GenomeSize",
#       "ChromosomeEnds",
...
#       "OutputGenesInterval",
#       "OutputGlobalFst",
#       "OutputGlobalFstStart",
#       "OutputGlobalFstInterval",
#       "OutputPairwiseFst",
#       "OutputPairwiseFstStart",
#       "OutputPairwiseFstInterval",
#       "OutputPerLocusFst",
#       "PatchList",
#       "NbrPatchesToSample",
#       "nIndividualsToSample",
#       "Stages"
#     ),
#     "Description" = c(
#       "Maximum size of genome (number of loci)",
#       "Positions where the genome is split into chromosomes (recombination probability = 0.5 between segments)",
#       "Recombination rate through chromosomal crossover across the whole genome",
#       "Output allele values for all genes of all sampled individuals?",
#       "Year at which gene value output starts",
#       "Interval (in years) for gene value output",
#       "Calculate global F-statistics (Weir & Cockerham 1984)?",
#       "Year at which global Fst output starts",
#       "Interval (in years) for global Fst output",
#       "Calculate pairwise patch F-statistics (Weir & Cockerham 1984)?",
#       "Year at which pairwise Fst output starts",
#       "Interval (in years) for pairwise Fst output",
#       "Calculate per-locus Fst estimates (requires OutputGlobalFst = TRUE)?",
#       "Patches to sample for genetic output: vector of patch IDs, 'all', 'random', or 'random_occupied'",
#       "Number of patches to sample randomly (only if PatchList = 'random' or 'random_occupied')",
#       "Number of individuals to sample per patch ('all' or positive integer)",
#       "Age stages to sample from (vector of integers or 'all')"
#     ),
#     "Values" = c(
#       fmt(gen@GenomeSize),
#       fmt(gen@ChromosomeEnds),
#       fmt(gen@RecombinationRate),
#       fmt(gen@OutputGeneValues),
#       fmt(gen@OutputGenesStart),
#       fmt(gen@OutputGenesInterval),
#       fmt(gen@OutputGlobalFst),
#       fmt(gen@OutputGlobalFstStart),
#       fmt(gen@OutputGlobalFstInterval),
#       fmt(gen@OutputPairwiseFst),
#       fmt(gen@OutputPairwiseFstStart),
#       fmt(gen@OutputPairwiseFstInterval),
#       fmt(gen@OutputPerLocusFst),
#       fmt(gen@PatchList),
#       fmt(gen@NbrPatchesToSample),
#       fmt(gen@nIndividualsToSample),
#       fmt(gen@Stages)
#     )
#   )
#  
#   # ---- NEUTRAL GENETICS ----
#   if(neutral_genetics) {
#     nt <<- gen@Traits@Neutral
#     NeutralGenetics <<- data.frame(
#       "Parameter" = c(
#         "Positions",
#         "NbOfPositions",
#         "InitialPositions",
#         "InitialNbOfPositions",
...
#       ),
#       "Description" = c(
#         "Loci positions coding for neutral trait within genome ('random' or vector of integers)",
#         "Number of randomly sampled loci positions (only if Positions = 'random')",
#         "Which positions to initialise: 'all', 'random', or vector of integers",
#         "Number of randomly sampled initial positions (only if InitialPositions = 'random')",
#         "Distribution to draw initial allele values from (uniform)",
#         "Maximum value for the uniform initial allele distribution (0-255)",
#         "Mutation model: KAM (k-alleles) or SSM (single-step mutation)",
#         "Maximum allele value for mutation distribution (0-255)",
#         "Mutation rate per locus (0.0 - 1.0)",
#         "Write allele values for this trait to output?"
#       ),
#       "Values" = c(
#         fmt(nt@Positions),
#         fmt(nt@NbOfPositions),
#         fmt(nt@InitialPositions),
#         fmt(nt@InitialNbOfPositions),
#         fmt(nt@InitialAlleleDistribution),
#         fmt(nt@InitialAlleleParameters),
#         fmt(nt@MutationDistribution),
#         fmt(nt@MutationParameters),
#         fmt(nt@MutationRate),
#         fmt(nt@OutputValues)
#       )
#     )
...
#         "InitialAlleleDistribution",
#         "InitialAlleleParameters",
#         "InitialDomDistribution",
#         "InitialDomParameters",
...
#       ),
#       "Description" = c(
#         "Number of genetic loads (up to 5)",
#         "Loci positions for each genetic load ('random' or vector of integers, as list)",
#         "Number of randomly sampled positions per genetic load (only if Positions = 'random')",
#         "Positions to initialise per load: 'all', 'random', or vector of integers (as list)",
#         "Number of initial random positions per load (only if InitialPositions = 'random')",
#         "Distribution for initial allele values: gamma, uniform, normal, negExp (per load)",
#         "Parameters for initial allele distribution (matrix, one row per load)",
#         "Distribution for initial dominance values: gamma, uniform, normal, negExp, scaled (per load)",
#         "Parameters for initial dominance distribution (matrix, one row per load)",
#         "Distribution for dominance coefficients: gamma, uniform, normal, negExp, scaled (per load)",
#         "Parameters for dominance distribution (matrix, one row per load)",
#         "Mutation distribution per load: normal, gamma, uniform, negExp",
#         "Parameters for mutation distribution (matrix, one row per load)",
#         "Mutation rate per locus (0.0 - 1.0)",
#         "Write allele values for this trait to output? (logical per load)"
#       ),
#       "Values" = c(
#         fmt(gl@NbGeneticLoads),
#         fmt(gl@Positions),
#         fmt(gl@NbOfPositions),
#         fmt(gl@InitialPositions),
#         fmt(gl@InitialNbOfPositions),
#         fmt(gl@InitialAlleleDistribution),
#         fmt(gl@InitialAlleleParameters),
#         fmt(gl@InitialDomDistribution),
#         fmt(gl@InitialDomParameters),
#         fmt(gl@DominanceDistribution),
#         fmt(gl@DominanceParameters),
#         fmt(gl@MutationDistribution),
#         fmt(gl@MutationParameters),
#         fmt(gl@MutationRate),
#         fmt(gl@OutputValues)
#       )
#     )
...
#       ),
#       "Description" = c(
#         "Loci positions coding for emigration traits within genome ('random' or vector of integers)",
#         "Number of randomly sampled positions (only if Positions = 'random')",
#         "How alleles are combined to produce trait value: 'additive' or 'average'",
#         "Are traits heritable (TRUE) or only showing individual variability (FALSE)?",
#         "Distribution for initial allele values: normal, gamma, uniform, negExp",
#         "Parameters for initial allele distribution (matrix)",
#         "Mutation distribution: normal, gamma, uniform, negExp",
#         "Parameters for mutation distribution (matrix)",
#         "Mutation rate per locus (0.0 - 1.0)",
#         "Write allele values for this trait to output?"
#       ),
#       "Values" = c(
#         fmt(eg@Positions),
#         fmt(eg@NbOfPositions),
#         fmt(eg@ExpressionType),
#         fmt(eg@IsInherited),
#         fmt(eg@InitialAlleleDistribution),
#         fmt(eg@InitialAlleleParameters),
#         fmt(eg@MutationDistribution),
#         fmt(eg@MutationParameters),
#         fmt(eg@MutationRate),
#         fmt(eg@OutputValues)
#       )
#     )
...
#       ),
#       "Description" = c(
#         "Loci positions coding for settlement traits within genome ('random' or vector of integers)",
#         "Number of randomly sampled positions (only if Positions = 'random')",
#         "How alleles are combined to produce trait value: 'additive' or 'average'",
#         "Are traits heritable (TRUE) or only showing individual variability (FALSE)?",
#         "Distribution for initial allele values: normal, gamma, uniform, negExp",
...
#       "Values" = c(
#         fmt(sg@Positions),
#         fmt(sg@NbOfPositions),
#         fmt(sg@ExpressionType),
#         fmt(sg@IsInherited),
#         fmt(sg@InitialAlleleDistribution),
#         fmt(sg@InitialAlleleParameters),
#         fmt(sg@MutationDistribution),
#         fmt(sg@MutationParameters),
#         fmt(sg@MutationRate),
#         fmt(sg@OutputValues)
#       )
#     )
#   }
#  
#   # ---- TRANSFER GENES (CorrRW / SMS / Kernel) ----
#   if(transfer_genes) {
#  
#     if(!is.logical(gen@Traits@CorrRWGenes)) {
#       tg <<- gen@Traits@CorrRWGenes
#       transfer_gene_type <<- "CorrRW"
#     } else if(!is.logical(gen@Traits@SMSGenes)) {
#       tg <<- gen@Traits@SMSGenes
#       transfer_gene_type <<- "SMS"
#     } else {
#       tg <<- gen@Traits@KernelGenes
#       transfer_gene_type <<- "Kernel"
#     }
#  
...
#       ),
#       "Description" = c(
#         "Transfer gene type (CorrRW, SMS or Kernel)",
#         "Loci positions coding for transfer traits within genome ('random' or vector of integers)",
#         "Number of randomly sampled positions (only if Positions = 'random')",
#         "How alleles are combined to produce trait value: 'additive' or 'average'",
#         "Are traits heritable (TRUE) or only showing individual variability (FALSE)?",
#         "Distribution for initial allele values: normal, gamma, uniform, negExp",
...
#         transfer_gene_type,
#         fmt(tg@Positions),
#         fmt(tg@NbOfPositions),
#         fmt(tg@ExpressionType),
#         fmt(tg@IsInherited),
#         fmt(tg@InitialAlleleDistribution),
#         fmt(tg@InitialAlleleParameters),
#         fmt(tg@MutationDistribution),
#         fmt(tg@MutationParameters),
#         fmt(tg@MutationRate),
#         fmt(tg@OutputValues)
#       )
#     )
...
# ============================================================
# Render tables (add to knitr section at the bottom)
# ============================================================
 
# if(genetics) {
#   Genetics <<- Genetics[Genetics$Values != "NULL", ]
#   rownames(Genetics) <<- NULL
#   knitr::kable(Genetics, col.names = gsub("[.]", " ", names(Genetics)), caption = 'Genetics parameters')
# }
# if(neutral_genetics) {
#   NeutralGenetics <<- NeutralGenetics[NeutralGenetics$Values != "NULL", ]
#   rownames(NeutralGenetics) <<- NULL
#   knitr::kable(NeutralGenetics, col.names = gsub("[.]", " ", names(NeutralGenetics)), caption = 'Neutral genetics parameters')
# }
# if(geneticload) {
#   GeneticLoad <<- GeneticLoad[GeneticLoad$Values != "NULL", ]
#   rownames(GeneticLoad) <<- NULL
#   knitr::kable(GeneticLoad, col.names = gsub("[.]", " ", names(GeneticLoad)), caption = 'Genetic load parameters')
# }
# if(emigration_genes) {
#   EmigrationGenes <<- EmigrationGenes[EmigrationGenes$Values != "NULL", ]
#   rownames(EmigrationGenes) <<- NULL
#   knitr::kable(EmigrationGenes, col.names = gsub("[.]", " ", names(EmigrationGenes)), caption = 'Emigration gene parameters')
# }
# if(settlement_genes) {
#   SettlementGenes <<- SettlementGenes[SettlementGenes$Values != "NULL", ]
#   rownames(SettlementGenes) <<- NULL
#   knitr::kable(SettlementGenes, col.names = gsub("[.]", " ", names(SettlementGenes)), caption = 'Settlement gene parameters')
# }
# if(transfer_genes) {
#   TransferGenes <<- TransferGenes[TransferGenes$Values != "NULL", ]
#   rownames(TransferGenes) <<- NULL
#   knitr::kable(TransferGenes, col.names = gsub("[.]", " ", names(TransferGenes)), caption = paste('Transfer gene parameters (', transfer_gene_type, ')'))
# }
