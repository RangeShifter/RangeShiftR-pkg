...
#'
#' @description Set genetics parameters and control genetic architecture.\cr
#'
#' Controls heritability and evolution of traits (if inter-individual variability (\code{IndVar}) is enabled for at least one (dispersal) trait).
...
#' @details When interindividual variability in traits (at present limited to dispersal traits) is modelled, each individual carries a genome coding for the
#' varying traits. \cr If the reproductive model is asexual or female-only, the species is assumed to be haploid and chromosomes
#' hold a single allele at each locus. Changes in the genotype, and hence also in the phenotype, can occur only through mutation.
#' In the case of sexual models, the species is assumed to be diploid and chromosomes hold two alleles at each locus.
#' New-born offspring inherit one set of chromosomes from each of its parents.
...
#'
#' \emph{Simple or Flexible genetic architecture}\cr
...
#' Example architecture file contents to show the required format:\cr\cr
#' NChromosomes	4\cr
#' NLoci	5 6 10 2\cr
#' Trait 0 NLoci 4 0 0 0 1 0 2 1 5\cr
#' Trait 1 NLoci 8 0 4 2 0 2 1 2 2 2 3 2 4 2 9 3 0\cr
#' Trait 2 NLoci 2 1 4 3 1 \cr\cr
#' The second line contains the number of loci on each chromosome.
#' Then, Traits are specified in the order they are defined in the model (emigration / movement / settlement),
#' and each trait is mapped to a number of loci, which is given after the keyword \code{NLoci}, followied by
#' this many number pairs specifying the respective chromosome and locus. (Note that numbering starts at \eqn{0}.)
...
#' genetics being set up for the species.\cr\cr
...
#' to the phenotypic scale. A scaling factor is specified for each trait (the parameter \code{MutationScales})
#' in each dispersal sub-module), which governs how large a
...
#' sub-modules, where they replace the constant values used when \code{IndVar = FALSE}.\cr
#' The standard deviation for a trait may not be greater than the corresponding
#' scaling factor (\code{MutationScales}), but it may be substantially less if an initial population which is highly homogeneous
...
#' that a mutation will take place, and standard deviation \code{MutationSD}. They
#' are applied in a standard way at the level of the individual locus (unlike in version 1, in
...
#' The crossover probability, \code{ProbCross}, applies only to diploid genomes, and is the probability that,
...
#' parent’s paternally-inherited alleles or vice versa. It is applied at the scale of the locus, i.e. during meiosis.
#' As a parent’s chromosomes are being inherited by its offspring, a crossover occurs at each locus with the
#' specified probability. If the crossover probability is high, the degree of linkage is reduced, and two linked
#' traits tend to evolve more independently.
#'
#'
#' @references ???
#' @name Genetics
