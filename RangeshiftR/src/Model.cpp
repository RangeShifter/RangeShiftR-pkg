...
//	<< endl;
		...
		if (pred.id != predcomp.id) { // save comparison for previous id
			if (nreps > 0) meanpred /= (double)nreps;
			else meanpred = 0.0;
...
#endif
	{
		if (d.diploid) outPar << "DIPLOID" << endl;
...
//	if (c.edgeBias) outPar << "yes"; else outPar << "no"; outPar << endl;
	...
	if (ppLand.dmgLoaded) {
		...
		if (d.type > 0) {
			...
			switch (d.occOption) {
			case 0:
				outPar << "total population size, ";
				break;
			case 1:
				outPar << "population density, ";
				break;
			case 2:
				outPar << "density of culled stages, ";
				break;
			case 3:
...
//	if (sim.outStartOcc > 0) outPar << " starting year " << sim.outStartOcc;
	...
	if (sim.outTraitsCells) {
		outPar << "Traits per ";
		if (ppLand.patchModel) outPar << "patch";
		else outPar << "cell";
