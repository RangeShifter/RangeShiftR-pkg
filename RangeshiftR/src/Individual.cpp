...
Individual* Individual::getMate(void)
{
	return pMate;
}
#endif
...
#else
	if (father == 0) pFatherGenome = 0;
	else pFatherGenome = father->pGenome;
#endif // GROUPDISP 
...
#if VIRTUALECOLOGIST
Genome* Individual::getGenome(void)
{
	return pGenome;
}
#endif
...
#endif // PEDIGREE
void Individual::setGroupId(int g)
{
	if (g >= 0) groupId = g;
}
int Individual::getGroupId(void)
{
	return groupId;
}
#endif // GROUPDISP

int Individual::getSex(void)
{
	return sex;
}
...
#if SEASONAL
#if PARTMIGRN
int Individual::getMigrnStatus(void)
{
	return migrnstatus;
}
#endif // PARTMIGRN 
...
#if GOBYMODEL
bool Individual::isAsocial(void)
{
	return asocial;
}
#endif
...
Patch* Individual::getNatalPatch(void)
{
	return pNatalPatch;
}
...
//	<< endl;
	...
	if (smsData != 0) {
		s.dp = smsData->dp;
		s.gb = smsData->gb;
		s.alphaDB = smsData->alphaDB;
		s.betaDB = smsData->betaDB;
	}
#if RSDEBUG
//DEBUGLOG << "Individual::getSMSTraits(): indId=" << indId
//	<< " s.dp=" << s.dp << " s.gb=" << s.gb
...
void Individual::incFallow(void)
{
	fallow++;
}

void Individual::resetFallow(void)
{
	fallow = 0;
}
...
//	<< endl;
		...
		case 2: // CRW
			...
			if (steplen < 0.2*land.resol) steplen = 0.2*land.resol;
			rho = movt.rho;
...
//			}
						y2=999;
						x2=999; //to break out of x2 and y2 loops.
					}
					j++;
				}
			}
...
array3x3d Individual::calcWeightings(const double base,const double theta)
{
	...
	if (fabs(theta) > 7.0*M_PI/8.0) {
		dx = 0;
		dy = -1;
	} else {
		if (fabs(theta) > 5.0*M_PI/8.0) {
			dy = -1;
			if (theta > 0) dx = 1;
			else dx = -1;
		} else {
			if (fabs(theta) > 3.0*M_PI/8.0) {
				dy = 0;
				if (theta > 0) dx = 1;
				else dx = -1;
			} else {
				if (fabs(theta) > M_PI/8.0) {
					dy = 1;
					if (theta > 0) dx = 1;
					else dx = -1;
				} else {
...
// NW and SE corners of effective cost array relative to the current cell (x,y):
	...
	for (int x2=-1; x2<2; x2++) {   // index of relative move in x direction
		for (int y2=-1; y2<2; y2++) { // index of relative move in x direction
			...
			if (xmin > xmax) {
				int z=xmax;    // swap xmin and xmax
				xmax=xmin;
				xmin=z;
			}
			if (ymin > ymax) {
				int z=ymax;    // swap ymin and ymax
				ymax=ymin;
				ymin=z;
			}
				...
				for (int x3=xmin; x3<=xmax; x3++) {
					for (int y3=ymin; y3<=ymax; y3++) {
						...
						if ((x+x3) < 0) x4 = x+x3+land.maxX+1;
							...
							if ((x+x3) > land.maxX) x4 = x+x3-land.maxX-1;
							else x4 = x+x3;
						}
						if ((y+y3) < 0) y4 = y+y3+land.maxY+1;
						else {
							if ((y+y3) > land.maxY) y4 = y+y3-land.maxY-1;
							else y4 = y+y3;
						}
...
void Individual::setMatPosn(unsigned int pos)
{
	matPosn = pos;
}
// Get position in relatedness matrix
unsigned int Individual::getMatPosn(void)
{
	return matPosn;
}
#endif
