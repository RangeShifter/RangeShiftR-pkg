...
{
	minEast = origin.minEast;
	minNorth = origin.minNorth;
}
...
bool Landscape::habitatsIndexed(void)
{
	return habIndexed;
}
...
void Landscape::setAlpha(double a)
{
	if (a > 0.0) alpha = a;
}
double Landscape::getAlpha(void)
{
	return alpha;
}
...
int Landscape::numPatchChanges(void)
{
	return (int)patchchanges.size();
}
...
void Landscape::setDynamicLand(bool dyn)
{
	dynamic = dyn;
}
...
int Landscape::numLandChanges(void)
{
	return (int)landchanges.size();
}

landChange Landscape::getLandChange(short ix)
{
	landChange c;
	c.chgnum = c.chgyear = 0;
...
// set up bad float values to ensure that valid values are read
	float badhfloat = -9.0;
	if (habnodata == -9) badhfloat = -99.0;
	float badpfloat = -9.0;
	if (pchnodata == -9) badpfloat = -99.0;
...
// NB headers of all files have already been compared
	for (int i = 0; i < 2; i++) {
		...
		badfloat[i] = -9.0;
		if (mortnodata[i] == -9) badfloat[i] = -99.0;
	}
...
void Landscape::resetExtEvents(void)
{
	extevents.clear();
}

int Landscape::numExtEvents(void)
{
	return (int)extevents.size();
}
