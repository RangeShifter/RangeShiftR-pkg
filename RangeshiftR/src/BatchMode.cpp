...
// Check fixed model parameters
	...
	if (paramname == "BatchNum") {
		if (batchnum < 0) {
			BatchError(filetype,-999,19,"BatchNum");
			errors++;
	...
	if (paramname == "PatchModel") {
		if (patchmodel < 0 || patchmodel > 1) {
			BatchError(filetype,-999,1,"PatchModel");
			errors++;
	...
	if (paramname == "Resolution") {
		if (resolution < 1) {
			BatchError(filetype,-999,11,"Resolution");
			errors++;
	...
	if (paramname == "MaxHabitats") {
		if (landtype == 0) { // raster with unique habitat codes
			if (maxNhab < 2) {
				BatchError(filetype,-999,12,"MaxHabitats");
				errors++;
	...
	if (paramname == "SpeciesDist") {
		if (speciesdist < 0 || speciesdist > 1) {
			BatchError(filetype,-999,1,"SpeciesDist");
			errors++;
		} else {
	...
	if (paramname == "Reproduction") {
		...
		if (reproductn < 0 || reproductn > 3) {
			BatchError(filetype,-999,3,"Reproduction");
			errors++;
		}
#else
		if (reproductn < 0 || reproductn > 2) {
			BatchError(filetype,-999,2,"Reproduction");
			errors++;
		}
#endif
		else {
	...
	if (paramname == "RepSeasons") {
		if (repseasons < 1) {
			BatchError(filetype,-999,11,"RepSeasons");
			errors++;
	...
	if (paramname == "StageStruct") {
		...
		if (stagestruct < 0 || stagestruct > 1) {
			BatchError(filetype,-999,1,"StageStruct");
			errors++;
		}
#endif
	...
	if (paramname == "Transfer") {
		...
		if (transfer < 0 || transfer > 2) {
			BatchError(filetype,-999,2,"Transfer");
			errors++;
	...
	if (paramname == "ABCsamplesize") {
		if (nABCsamples < 1) {
			BatchError(filetype,-999,10,"ABCsamplesize");
			errors++;
		}
	} else controlFormatError = true; // wrong control file format
...
// Parse header line;
	...
	bParamFile >> header;
	if (header != "Replicates" ) errors++;
	bParamFile >> header;
	if (header != "Years" ) errors++;
#if SPATIALMORT
	bParamFile >> header;
	if (header != "MortChgYr" ) errors++;
#endif
	bParamFile >> header;
	if (header != "Absorbing" ) errors++;
	bParamFile >> header;
	if (header != "Gradient" ) errors++;
	bParamFile >> header;
	if (header != "GradSteep" ) errors++;
	bParamFile >> header;
	if (header != "Optimum" ) errors++;
	bParamFile >> header;
	if (header != "f" ) errors++;
	bParamFile >> header;
	if (header != "LocalExtOpt" ) errors++;
	bParamFile >> header;
	if (header != "Shifting" ) errors++;
	bParamFile >> header;
	if (header != "ShiftRate" ) errors++;
	bParamFile >> header;
	if (header != "ShiftStart" ) errors++;
	bParamFile >> header;
	if (header != "ShiftEnd" ) errors++;
	bParamFile >> header;
	if (header != "EnvStoch" ) errors++;
	bParamFile >> header;
	if (header != "EnvStochType" ) errors++;
#if BUTTERFLYDISP
	bParamFile >> header;
	if (header != "EnvStochFromFile" ) errors++;
	bParamFile >> header;
	if (header != "EnvStochFileName" ) errors++;
#endif
	bParamFile >> header;
	if (header != "ac" ) errors++;
	bParamFile >> header;
	if (header != "std" ) errors++;
	bParamFile >> header;
	if (header != "minR" ) errors++;
	bParamFile >> header;
	if (header != "maxR" ) errors++;
	bParamFile >> header;
	if (header != "minK" ) errors++;
	bParamFile >> header;
	if (header != "maxK" ) errors++;
	bParamFile >> header;
	if (header != "LocalExt" ) errors++;
	bParamFile >> header;
	if (header != "LocalExtProb" ) errors++;
#if BUTTERFLYDISP
	bParamFile >> header;
	if (header != "Dispersal" ) errors++;
#endif
	bParamFile >> header;
	if (header != "PropMales" ) errors++;
	bParamFile >> header;
	if (header != "Harem" ) errors++;
	bParamFile >> header;
	if (header != "bc" ) errors++;
	bParamFile >> header;
	if (header != "Rmax" ) errors++;
#if SEASONAL
...
// ADDITIONAL PARAMETERS FOR GROUP DISPERSAL MODEL
	bParamFile >> header;
	if (header != "Selfing" ) errors++;
	bParamFile >> header;
	if (header != "Paternity" ) errors++;
	bParamFile >> header;
	if (header != "PropLocal" ) errors++;
	bParamFile >> header;
	if (header != "PropNghbr" ) errors++;
#if PEDIGREE
	bParamFile >> header;
	if (header != "PedMatSize" ) errors++;
#endif // PEDIGREE
#endif // GROUPDISP 
#if SOCIALMODEL
// ADDITIONAL PARAMETERS FOR PROBIS SOCIAL POLYMORPHISM MODEL
	bParamFile >> header;
	if (header != "phenMean" ) errors++;
	bParamFile >> header;
	if (header != "phenSD" ) errors++;
	bParamFile >> header;
	if (header != "phenScale" ) errors++;
	bParamFile >> header;
	if (header != "ratioK" ) errors++;
	bParamFile >> header;
	if (header != "ratioRmax" ) errors++;
	bParamFile >> header;
	if (header != "ratioBc" ) errors++;
...
//bParamFile >> header; if (header != "Rs" ) errors++;
	...
	if (header != "dK" ) errors++;
	bParamFile >> header;
	if (header != "alpha" ) errors++;
#endif // SOCIALMODEL
	bParamFile >> header;
	if (header != "OutStartPop" ) errors++;
	bParamFile >> header;
	if (header != "OutStartInd" ) errors++;
	bParamFile >> header;
	if (header != "OutStartGenetic" ) errors++;
	bParamFile >> header;
	if (header != "OutStartTraitCell" ) errors++;
	bParamFile >> header;
	if (header != "OutStartTraitRow" ) errors++;
	bParamFile >> header;
	if (header != "OutStartConn" ) errors++;
#if RS_CONTAIN
	bParamFile >> header;
	if (header != "OutStartDamage" ) errors++;
#endif // RS_CONTAIN 
	bParamFile >> header;
	if (header != "OutIntRange" ) errors++;
	bParamFile >> header;
	if (header != "OutIntOcc" ) errors++;
	bParamFile >> header;
	if (header != "OutIntPop" ) errors++;
	bParamFile >> header;
	if (header != "OutIntInd" ) errors++;
	bParamFile >> header;
	if (header != "OutIntGenetic" ) errors++;
	bParamFile >> header;
	if (header != "OutGenType" ) errors++;
	bParamFile >> header;
	if (header != "OutGenCrossTab" ) errors++;
	bParamFile >> header;
	if (header != "OutIntTraitCell" ) errors++;
	bParamFile >> header;
	if (header != "OutIntTraitRow" ) errors++;
	bParamFile >> header;
	if (header != "OutIntConn" ) errors++;
#if RS_CONTAIN
	bParamFile >> header;
	if (header != "OutIntDamage" ) errors++;
#endif // RS_CONTAIN 
	bParamFile >> header;
	if (header != "SaveMaps" ) errors++;
	bParamFile >> header;
	if (header != "MapsInterval" ) errors++;
	bParamFile >> header;
	if (header != "SMSHeatMap" ) errors++;
	bParamFile >> header;
	if (header != "DrawLoadedSp" ) errors++;
...
// Parse data lines
	...
	if (inint < 0) {
		...
		prevsimul = firstsimul = inint;
		nSimuls++;
	}
	while (inint != -98765) {
		bParamFile >> replicates;
		if (replicates <= 0) {
			BatchError(filetype,line,11,"Replicates");
			errors++;
		}
		bParamFile >> years;
		if (years <= 0) {
			BatchError(filetype,line,11,"Years");
			errors++;
		}
#if SPATIALMORT
		bParamFile >> inint;
		if (landtype != 9 && inint <= 0) {
			BatchError(filetype,line,11,"MortChgYr");
			errors++;
		}
#endif
		bParamFile >> absorb;
		if (absorb < 0 || absorb > 1) {
			BatchError(filetype,line,1,"Absorbing");
			errors++;
		}
		bParamFile >> gradient;
		if (patchmodel) {
		...
		bParamFile >> infloat;
		if (gradient && infloat < 0.0) {
			BatchError(filetype,line,19,"GradSteep");
			errors++;
		}
		bParamFile >> inint;
		if (gradient && inint < 0) {
			BatchError(filetype,line,19,"Optimum");
			errors++;
		}
		bParamFile >> infloat;
		if (gradient && infloat < 0.0) {
			BatchError(filetype,line,19,"f");
			errors++;
		}
		bParamFile >> infloat;
		if (gradient == 4 && (infloat < 0.0 || infloat >= 1.0)) {
			BatchError(filetype,line,20,"LocalExtOpt");
			errors++;
		}
		bParamFile >> shifting;
		if (gradient && (shifting < 0 || shifting > 1)) {
			BatchError(filetype,line,1,"Shifting");
			errors++;
		}
		bParamFile >> infloat;
		if (gradient && shifting && infloat <= 0.0) {
			BatchError(filetype,line,10,"ShiftRate");
			errors++;
		}
		bParamFile >> shiftstart;
		if (gradient && shifting && shiftstart <= 0) {
			BatchError(filetype,line,10,"ShiftStart");
			errors++;
		}
		bParamFile >> shiftend;
...
//			envstoch = 0; // to prevent checking of subsequent fields
		...
		if (envstoch && (stochtype < 0 || stochtype > 1)) {
			BatchError(filetype,line,1,"EnvStochType");
			errors++;
		}
#if BUTTERFLYDISP
		ifstream bStochFile;
		...
		if (envstoch == 1) { // global stochasticity
			if (envstochfromfile < 0 || envstochfromfile > 1) {
				BatchError(filetype,line,1,"EnvStochFromFile");
				errors++;
			}
		} else {
		...
		if (envstochfromfile != 1) {
#endif
			if (envstoch && (infloat < 0.0 || infloat >= 1.0)) {
				BatchError(filetype,line,20,"ac");
				errors++;
			}
#if BUTTERFLYDISP
		}
...
#endif
			if (envstoch && (infloat <= 0.0 || infloat > 1.0)) {
				BatchError(filetype,line,20,"std");
				errors++;
			}
#if BUTTERFLYDISP
		}
#endif
		bParamFile >> minR;
		if (envstoch && stochtype == 0 && minR <= 0.0) {
			BatchError(filetype,line,10,"minR");
			errors++;
		}
		bParamFile >> maxR;
		...
		if (envstoch && stochtype == 1) {
			if (minK <= 0.0) {
				BatchError(filetype,line,10,"minK");
				errors++;
			}
			if (maxK <= minK) {
				BatchError(filetype,line,0," ");
		...
		if (patchmodel == 0 && localext == 1 && (infloat <= 0.0 || infloat >= 1.0)) {
			BatchError(filetype,line,20,"LocalExtProb");
			errors++;
		}
#if BUTTERFLYDISP
		bParamFile >> inint;
		if (stagestruct == 0 && (inint < 0 || inint > 1)) {
			BatchError(filetype,line,1,"Dispersal");
			errors++;
		}
#endif
		bParamFile >> infloat;
		if (reproductn && (infloat <= 0.0 || infloat >= 1.0)) {
			BatchError(filetype,line,20,"PropMales");
			errors++;
		}
		bParamFile >> infloat;
		if (reproductn == 2 && infloat <= 0.0) {
			BatchError(filetype,line,10,"Harem");
			errors++;
		}
		bParamFile >> infloat;
		if (stagestruct == 0 && infloat <= 0.0) {
			BatchError(filetype,line,10,"bc");
			errors++;
		}
		bParamFile >> infloat;
		if (stagestruct == 0 && infloat <= 0.0) {
			BatchError(filetype,line,10,"Rmax");
			errors++;
		}
#if SEASONAL
		for (int s = 0; s < nseasons; s++) {
			...
			for (i = 0; i < maxNhab; i++) {
				...
				if (infloat < 0.0) {
					...
					BatchError(filetype,line,19,Kheader);
					errors++;
				} else {
					sum_K += infloat;
					if (infloat > 0.0) {
...
// ADDITIONAL PARAMETERS FOR GROUP DISPERSAL MODEL
		bParamFile >> inint;
		if (inint < 0 || inint > 1) {
			BatchError(filetype,line,1,"Selfing");
			errors++;
		}
		int paternity;
		bParamFile >> paternity;
		if (paternity < 0 || paternity > 2) {
			BatchError(filetype,line,2,"Paternity");
			errors++;
		}
		...
		if (paternity == 2) {
			if (proplocal < 0.0 || proplocal > 1.0) {
				BatchError(filetype,line,20,"PropLocal");
				errors++;
			}
			if (propnghbr < 0.0 || propnghbr > 1.0) {
				BatchError(filetype,line,20,"PropNghbr");
				errors++;
			}
		...
		bParamFile >> inint;
		if (inint < 1 ) {
			BatchError(filetype,line,11,"PedMatSize");
			errors++;
		}
#endif // PEDIGREE
#endif // GROUPDISP 
...
//		>> soc.Ta >> soc.Ts >> soc.Ca >> soc.Cs >> soc.dK >> soc.alpha;
		...
		if (soc.socSD <= 0.0) {
			BatchError(filetype,line,10,"phenSD");
			errors++;
		}
		if (soc.socScale <= 0.0) {
			BatchError(filetype,line,10,"phenScale");
			errors++;
		}
		if (soc.socSD > soc.socScale) {
			BatchError(filetype,line,3,"phenSD","phenScale");
			errors++;
		}
		if (soc.asocK <= 0.0) {
			BatchError(filetype,line,10,"ratioK");
			errors++;
		}
		if (soc.asocRmax <= 0.0) {
			BatchError(filetype,line,10,"ratioRmax");
			errors++;
		}
		if (soc.asocBc <= 0.0) {
			BatchError(filetype,line,10,"ratioBc");
			errors++;
		}
//	if (soc.ra <= 0.0) {
...
//	}
		if (soc.Ta <= 0.0 || soc.Ta >= 1.0) {
			BatchError(filetype,line,20,"Ta");
			errors++;
		}
		if (soc.Ts <= 0.0 || soc.Ts >= 1.0) {
			BatchError(filetype,line,20,"Ts");
			errors++;
		}
//	if (soc.Ca <= 0.0) {
...
//	}
		if (soc.dK <= 0.0 || soc.dK >= 1.0) {
			BatchError(filetype,line,20,"dK");
			errors++;
		}
		if (soc.alpha <= 0.0) {
			BatchError(filetype,line,10,"alpha");
			errors++;
		}
#endif // SOCIALMODEL
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutStartPop");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutStartInd");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutStartGenetic");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutStartTraitCell");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutStartTraitRow");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutStartConn");
			errors++;
		}
#if RS_CONTAIN
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutStartDamage");
			errors++;
		}
#endif // RS_CONTAIN 
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntRange");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntOcc");
			errors++;
		} else {
		...
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntPop");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntInd");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntGenetic");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0 || inint > 2) {
			BatchError(filetype,line,2,"OutGenType");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0 || inint > 1) {
			BatchError(filetype,line,1,"OutGenCrossTab");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntTraitCell");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntTraitRow");
			errors++;
		}
		bParamFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"OutIntConn");
			errors++;
		} else {
		...
		if (landtype == 9) {
			...
			if (inint < 0) {
				BatchError(filetype,line,19,"OutIntDamage");
				errors++;
			}
		}
#endif // RS_CONTAIN 
		bParamFile >> savemaps;
		if (savemaps < 0 || savemaps > 1) {
			BatchError(filetype,line,1,"SaveMaps");
			errors++;
		}
		bParamFile >> inint;
		if (savemaps == 1 && inint < 1) {
...
//batchlog << "ParseLandFile(): starting " << endl;
	if (landtype == 0 || landtype == 2) { // real landscape
		// Parse header line;
		bLandFile >> header;
		if (header != "LandNum" ) errors++;
//	batchlog << "ParseLandFile(): header = " << header << endl;
		bLandFile >> header;
		if (header != "Nhabitats" ) errors++;
		bLandFile >> header;
		if (header != "LandscapeFile" ) errors++;
		bLandFile >> header;
		if (header != "PatchFile" ) errors++;
		bLandFile >> header;
		if (header != "DynLandFile" ) errors++;
		bLandFile >> header;
		if (header != "SpDistFile" ) errors++;
#if RS_CONTAIN
		bLandFile >> header;
		if (header != "DamageFile" ) errors++;
#endif // RS_CONTAIN 
#if SPATIALMORT
		bLandFile >> header;
		if (header != "MortFile1" ) errors++;
		bLandFile >> header;
		if (header != "MortFile2" ) errors++;
#endif
		if (errors > 0) {
...
//		 batchlog << "ParseLandFile(): Landscape no. = " << inint << endl;
			if (inint < 1) {
				...
				for (j = 0; j < (int)landlist.size(); j++) {
					if (inint == landlist[j]) {
						BatchError(filetype,line,666,"LandNum");
						j = landlist.size() + 1;
						errors++;
					}
				}
...
//			<< " to landscape list" << endl;
			...
			if (landtype == 0) { // raster map with unique habitat codes
				if (inint < 0) {
					BatchError(filetype,line,10,"Nhabitats");
					errors++;
				}
				if (inint > maxNhab) {
					BatchError(filetype,line,0," ");
...
//		batchlog << "ParseLandFile(): first item of next line = " << inint << endl;
		...
		if (landtype == 9) { // artificial landscape
			...
			if (header != "LandNum" ) errors++;
			bLandFile >> header;
			if (header != "Fractal" ) errors++;
			bLandFile >> header;
			if (header != "Type" ) errors++;
			bLandFile >> header;
			if (header != "Xdim" ) errors++;
			bLandFile >> header;
			if (header != "Ydim" ) errors++;
			bLandFile >> header;
			if (header != "MinHab" ) errors++;
			bLandFile >> header;
			if (header != "MaxHab" ) errors++;
			bLandFile >> header;
			if (header != "Psuit" ) errors++;
			bLandFile >> header;
			if (header != "H" ) errors++;
			if (errors > 0) {
				FormatError(filetype,0);
...
//			<< " to landscape list" << endl;
				...
				if (fractal < 0 || fractal > 1) {
					BatchError(filetype,line,1,"Fractal");
					errors++;
				}
				bLandFile >> type;
				if (type < 0 || type > 1) {
					BatchError(filetype,line,1,"Type");
					errors++;
				}
				bLandFile >> Xdim >> Ydim;
				if (fractal == 1) {
					if (Xdim < 3) {
						BatchError(filetype,line,13,"Xdim");
						errors++;
					}
					if (Ydim < 3) {
						BatchError(filetype,line,13,"Ydim");
						errors++;
					}
				} else {
					if (Xdim < 1) {
						BatchError(filetype,line,11,"Xdim");
						errors++;
					}
					if (Ydim < 1) {
						BatchError(filetype,line,11,"Ydim");
						errors++;
					}
				}
				if (fractal == 1) {
					if (Ydim < Xdim) {
						BatchError(filetype,line,0," ");
						batchlog << "Y dimension may not be less than X dimension" << endl;
						errors++;
					}
					if ((Xdim > 2 && power2check(Xdim-1) != 1)
					    ||  (Ydim > 2 && power2check(Ydim-1) != 1)) {
						BatchError(filetype,line,0," ");
						batchlog << "X and Y dimensions must be a power of 2 plus 1" << endl;
						errors++;
					}
				}
				...
				if (type == 1) { // continuous landscape
					if (minhab <= 0.0 || minhab >= 100.0) {
						BatchError(filetype,line,100,"MinHab");
						errors++;
					}
					if (maxhab <= 0.0 || maxhab > 100.0) {
						BatchError(filetype,line,100,"MaxHab");
						errors++;
					}
					if (maxhab <= minhab) {
						BatchError(filetype,line,0," ");
						batchlog << "MaxHab must exceed MinHab" << endl;
						errors++;
					}
				}
				bLandFile >> infloat;
				if (infloat < 0.0 || infloat > 1.0) {
					BatchError(filetype,line,20,"Psuit");
					errors++;
				}
				bLandFile >> infloat;
				if (fractal == 1) {
					if (infloat <= 0.0 || infloat >= 1.0) {
						BatchError(filetype,line,20,"H");
						errors++;
					}
				}
				totlines++;
...
//int totlines = 0;
	...
	bDynLandFile >> header;
	if (header != "Change" ) errors++;
	bDynLandFile >> header;
	if (header != "Year" ) errors++;
	bDynLandFile >> header;
	if (header != "LandChangeFile" ) errors++;
	bDynLandFile >> header;
	if (header != "PatchChangeFile" ) errors++;
...
// Parse data lines
	...
	while (change != -98765) {
		...
		bDynLandFile >> year;
		if (year <= 0) {
			BatchError(filetype,line,10,"Year");
			errors++;
		}
		if (line > 1) {
			if (year <= prevyear) {
				BatchError(filetype,line,1,"Year","previous Year");
				errors++;
			}
		}
		prevyear = year;
...
#if GOBYMODEL
	bStageStructFile >> header;
	if (header != "InitPhenMean" ) errors++;
	bStageStructFile >> header;
	if (header != "InitPhenSD" ) errors++;
	bStageStructFile >> header;
	if (header != "InitPhenScale" ) errors++;
	bStageStructFile >> header;
	if (header != "Fasocial" ) errors++;
#endif
	bStageStructFile >> header;
	if (header != "PostDestructn" ) errors++;
	bStageStructFile >> header;
	if (header != "PRep" ) errors++;
	bStageStructFile >> header;
	if (header != "RepInterval" ) errors++;
	bStageStructFile >> header;
	if (header != "MaxAge" ) errors++;
#if RS_CONTAIN
	bStageStructFile >> header;
	if (header != "HabDemFile" ) errors++;
	bStageStructFile >> header;
	if (header != "TransMatrixFile" ) errors++;
#else
#if SEASONAL
	bStageStructFile >> header;
	if (header != "SeasonFile" ) errors++;
#else
	bStageStructFile >> header;
	if (header != "TransMatrixFile" ) errors++;
#endif // SEASONAL 
#endif // RS_CONTAIN 
	bStageStructFile >> header;
	if (header != "SurvSched" ) errors++;
	bStageStructFile >> header;
	if (header != "FecDensDep" ) errors++;
	bStageStructFile >> header;
	if (header != "FecStageWts" ) errors++;
	bStageStructFile >> header;
	if (header != "FecStageWtsFile" ) errors++;
	bStageStructFile >> header;
	if (header != "DevDensDep" ) errors++;
	bStageStructFile >> header;
	if (header != "DevDensCoeff" ) errors++;
	bStageStructFile >> header;
	if (header != "DevStageWts" ) errors++;
	bStageStructFile >> header;
	if (header != "DevStageWtsFile" ) errors++;
	bStageStructFile >> header;
	if (header != "SurvDensDep" ) errors++;
	bStageStructFile >> header;
	if (header != "SurvDensCoeff" ) errors++;
	bStageStructFile >> header;
	if (header != "SurvStageWts" ) errors++;
	bStageStructFile >> header;
	if (header != "SurvStageWtsFile" ) errors++;
#if PARTMIGRN
	bStageStructFile >> header;
...
// first simulation number must match first one in parameterFile
	if (inint != firstsimul) {
		BatchError(filetype,line,111,"Simulation");
		errors++;
	}
	prevsimul = inint;
	while (inint != -98765) {
		...
		if (initPhenSD <= 0.0) {
			BatchError(filetype,line,10,"InitPhenSD");
			errors++;
		}
		bStageStructFile >> initPhenScale;
		if (initPhenScale <= 0.0) {
			BatchError(filetype,line,10,"InitPhenScale");
			errors++;
		}
		if (initPhenSD > initPhenScale) {
			BatchError(filetype,line,3,"InitPhenSD","InitPhenScale");
			errors++;
		}
		bStageStructFile >> fasocial;
		if (fasocial <= 0.0) {
			BatchError(filetype,line,10,"Fasocial");
			errors++;
		}
#endif
		bStageStructFile >> inint;
		if (inint < 0 || inint > 1) {
			BatchError(filetype,line,1,"PostDestructn");
			errors++;
		}
		bStageStructFile >> infloat;
		if (infloat <= 0 || infloat > 1.0) {
			BatchError(filetype,line,20,"PRep");
			errors++;
		}
		bStageStructFile >> inint;
		if (inint < 0) {
			BatchError(filetype,line,19,"RepInterval");
			errors++;
		}
		bStageStructFile >> inint;
		if (inint < 2) {
			BatchError(filetype,line,12,"MaxAge");
			errors++;
		}
...
//				<< " transfiles[i] = " << transfiles[i] << endl;
...
#if SEASONAL
		if (inint < 0 || inint > 1) {
			BatchError(filetype,line,1,"SurvSched");
			errors++;
		}
#else
		if (inint < 0 || inint > 2) {
			BatchError(filetype,line,2,"SurvSched");
			errors++;
		}
#endif
		bStageStructFile >> fecdensdep;
		if (fecdensdep < 0 || fecdensdep > 1) {
			BatchError(filetype,line,1,"FecDensDep");
			errors++;
			fecdensdep = 1;
		}
		...
		if (fecdensdep) {
			if (fecstagewts < 0 || fecstagewts > 1) {
				BatchError(filetype,line,1,"FecStageWts");
				errors++;
				fecstagewts = 1;
			}
		} else {
			if (fecstagewts != 0) {
				BatchError(filetype,line,0," ");
				batchlog << "FecStageWts must be 0 if FecDensDep is 0" << endl;
				errors++;
				errors++;
				fecstagewts = 1;
			}
		...
		if (devdensdep < 0 || devdensdep > 1) {
			BatchError(filetype,line,1,"DevDensDep");
			errors++;
			devdensdep = 1;
		}
		...
		if (devdensdep) {
			if (infloat <= 0.0) {
				BatchError(filetype,line,10,"DevDensCoeff");
				errors++;
			}
			if (devstagewts < 0 || devstagewts > 1) {
				BatchError(filetype,line,1,"DevStageWts");
				errors++;
				devstagewts = 1;
			}
		} else {
			if (devstagewts != 0) {
				BatchError(filetype,line,0," ");
				batchlog << "DevStageWts must be 0 if DevDensDep is 0" << endl;
				errors++;
				errors++;
				devstagewts = 1;
			}
		...
		if (survdensdep < 0 || survdensdep > 1) {
			BatchError(filetype,line,1,"SurvDensDep");
			errors++;
			survdensdep = 1;
		}
		...
		if (survdensdep) {
			if (infloat <= 0.0) {
				BatchError(filetype,line,10,"SurvDensCoeff");
				errors++;
			}
			if (survstagewts < 0 || survstagewts > 1) {
				BatchError(filetype,line,1,"SurvStageWts");
				errors++;
				survstagewts = 1;
			}
		} else {
			if (survstagewts != 0) {
				BatchError(filetype,line,0," ");
				batchlog << "SurvStageWts must be 0 if SurvDensDep is 0" << endl;
				errors++;
				errors++;
				survstagewts = 1;
			}
		...
		for (i = 0; i < 6; i++) {
			...
			if (prop < 0.0 || prop > 1.0) {
				BatchError(filetype,line,20,"Proportion Phil/Disp Res/Mig");
				errors++;
			}
		}
		...
		if (resetmigrn < 0 || resetmigrn > 1) {
			BatchError(filetype,line,1,"ResetMigrn");
			errors++;
		}
#endif // PARTMIGRN 
	...
	for (i = 0; i < nstages; i++) {
		for (j = 0; j < nsexesDem; j++) {
			...
			if (nsexesDem == 1) hhh = Int2Str(i);
				...
				if (j == 0) hhh = Int2Str(i) + "m";
				else hhh = Int2Str(i) + "f";
			}
...
// single row for juveniles
	...
	if (header != "0" ) {
		BatchError(filetype,line,0," ");
		batchlog << "Invalid row header" << endl;
		errors++;
	}
	...
	for (i = 0; i < nstages; i++) {
		for (j = 0; j < nsexesDem; j++) {
			...
			if (i > 0) {
				if (infloat < 0.0) {
					BatchError(filetype,line,19,"Fecundity");
					errors++;
				}
...
#if !SEASONAL
	if (totfecundity <= 0.0) {
		BatchError(filetype,line,10,"Total fecundity");
		errors++;
	}
#endif
	bTransMatrix >> minage;
...
//				batchlog << "MINAGE = " << minage << endl;
	if (minage != 0) {
		BatchError(filetype,line,0," ");
		batchlog << "MinAge must be zero for juvenile stage" << endl;
		errors++;
	}
	...
	for (stage = 1; stage < nstages; stage++) {
		for (sex = 0; sex < nsexesDem; sex++) {
			...
			if (nsexesDem == 1) hhh = Int2Str(stage);
				...
				if (sex == 0) hhh = Int2Str(stage) + "m";
				else hhh = Int2Str(stage) + "f";
			}
			if (header != hhh) {
				BatchError(filetype,line,0," ");
				batchlog << "Invalid row header" << endl;
...
//				batchlog << "TRANS PROB = " << infloat << endl;
					if (infloat < 0.0 || infloat > 1) {
						BatchError(filetype,line,20,"Transition probability");
						errors++;
					}
				}
			}
...
//				batchlog << "MINAGE = " << minage << endl;
			if (stage == 1 && minage != 0) {
				BatchError(filetype,line,0," ");
				batchlog << "MinAge must be zero for stage 1" << endl;
				errors++;
			}
			if (stage > 1) {
				if (minage < 0) {
...
// Parse header line;
	bHabDemFile >> header;
	if (header != "HabCode" ) errors++;
#if SEASONAL
	bHabDemFile >> header;
	if (header != "Season" ) errors++;
#endif // SEASONAL 
	bHabDemFile >> header;
	if (header != "TransMatrixFile" ) errors++;
	if (errors > 0) {
...
//prevseason = inint;
	while (inint != -98765) {
		...
		if (inint < 1 || inint > maxcode) {
			BatchError(filetype,line,0," ");
			batchlog << "Habitat code must be from 1 to " << maxcode << endl;
			errors++;
		}
#if SEASONAL
		bHabDemFile >> inint;
...
//	}
		if (inint < 0 || inint >= nseasons) {
			BatchError(filetype,line,0," ");
			batchlog << "Season must be from 0 to " << nseasons-1 << endl;
			errors++;
		}
#endif // SEASONAL 
		// transition matrix file - compulsory
...
// Parse header line;
	...
	if (header != "TransMatrixFile" ) errors++;
	bSeasonFile >> header;
	if (header != "ProbExtreme" ) errors++;
	bSeasonFile >> header;
	if (header != "MortExtreme" ) errors++;
	if (errors > 0) {
		FormatError(filetype,errors);
		return -111;
	}
...
// first season number must be zero
	if (inint != 0) {
		BatchError(filetype,line,0," ");
		batchlog << "First season must be 0" << endl;
		errors++;
	}
	prevseason = inint;
	while (inint != -98765) {
...
//				<< " transfiles[i] = " << transfiles[i] << endl;
		...
		bSeasonFile >> infloat;
		if (infloat < 0.0 || infloat > 1.0) {
			BatchError(filetype,line,20,"ProbExtreme");
			errors++;
		}
		bSeasonFile >> infloat;
		if (infloat < 0.0 || infloat > 1.0) {
			BatchError(filetype,line,20,"MortExtreme");
			errors++;
		}
...
// Parse data lines
	...
	while (year != -98765) {
		...
		if (season >= nseasons) {
			BatchError(filetype,line,4,"Season","NSeasons in Control file");
			errors++;
		}
		if (year == prevyear) {
			if (season < prevseason) {
				BatchError(filetype,line,2,"Season","previous Season");
				errors++;
			}
		}
		prevyear = year;
...
int ParseEmigFile(void)
{
	...
	float epScale0,d0Scale0,alphaScale0,betaScale0;
	int errors = 0;
	int simuls = 0;
	string filetype = "EmigrationFile";

// Parse header line;
	bEmigrationFile >> header;
	if (header != "Simulation" ) errors++;
	bEmigrationFile >> header;
	if (header != "DensDep" ) errors++;
	bEmigrationFile >> header;
	if (header != "UseFullKern" ) errors++;
	bEmigrationFile >> header;
	if (header != "StageDep" ) errors++;
	bEmigrationFile >> header;
	if (header != "SexDep" ) errors++;
	bEmigrationFile >> header;
	if (header != "IndVar" ) errors++;
	bEmigrationFile >> header;
	if (header != "EmigStage" ) errors++;
	bEmigrationFile >> header;
...
// ADDITIONAL PARAMETERS FOR GROUP DISPERSAL MODEL
	bEmigrationFile >> header;
	if (header != "GroupDisp" ) errors++;
	bEmigrationFile >> header;
	if (header != "GroupMean" ) errors++;
#endif
#if GOBYMODEL
	bEmigrationFile >> header;
	if (header != "Dasocial" ) errors++;
#endif
	bEmigrationFile >> header;
	if (header != "EP" ) errors++;
	bEmigrationFile >> header;
	...
	if (header != "alpha" ) errors++;
	bEmigrationFile >> header;
	if (header != "beta" ) errors++;
	bEmigrationFile >> header;
	if (header != "EPMean" ) errors++;
	bEmigrationFile >> header;
	if (header != "EPSD" ) errors++;
	bEmigrationFile >> header;
	if (header != "D0Mean" ) errors++;
	bEmigrationFile >> header;
	if (header != "D0SD" ) errors++;
	bEmigrationFile >> header;
	if (header != "alphaMean" ) errors++;
	bEmigrationFile >> header;
	if (header != "alphaSD" ) errors++;
	bEmigrationFile >> header;
	if (header != "betaMean" ) errors++;
	bEmigrationFile >> header;
	if (header != "betaSD" ) errors++;
	bEmigrationFile >> header;
	if (header != "EPScale" ) errors++;
	bEmigrationFile >> header;
	if (header != "D0Scale" ) errors++;
	bEmigrationFile >> header;
	if (header != "alphaScale" ) errors++;
	bEmigrationFile >> header;
	if (header != "betaScale" ) errors++;
	if (errors > 0) {
		FormatError(filetype,errors);
		return -111;
	}
...
// first simulation number must match first one in parameterFile
	...
	while (simul != -98765) {
		...
		if (densdep < 0 || densdep > 1) {
			BatchError(filetype,line,1,"DensDep");
			errors++;
		}
		// validate use full kernel
		if (usefullkern < 0 || usefullkern > 1) {
			BatchError(filetype,line,1,"UseFullKern");
			errors++;
		}
		if (densdep != 0) {
			if (usefullkern != 0) {
				BatchError(filetype,line,0,"UseFullKern");
				errors++;
		...
		if (stagestruct && !stagedep && indvar) {
			if (stage == 0 && sex == 0) {
				if (emigstage < 0 || emigstage >= stages) {
					BatchError(filetype,line,0,"EmigStage");
					errors++;
		...
		if (stage == 0 && sex == 0) { // first line of a simulation
			...
			if (densdep == 1) densdepset = true;
			else densdepset = false;
			if (indvar == 1)  indvarset = true;
			else indvarset = false;
		}
...
// ADDITIONAL PARAMETERS FOR GROUP DISPERSAL MODEL
		...
		if (stage == 0 && sex == 0) { // first line of a simulation
			if (groupdisp < 0 || groupdisp > 1) {
				BatchError(filetype,line,1,"GroupDisp");
				errors++;
			}
			if (groupdisp == 1) {
				if (groupmean <= 1.0) {
		...
		if (densdep == 1 && stage == 0 && sex == 0) { // first line of a DD simulation
			if (asocD <= 0.0) {
				BatchError(filetype,line,10,"Dasocial");
				errors++;
			}
		}
#endif // GOBYMODEL
...
//	<< endl;
		...
		if (current.newsimul) {
			...
			epScale0 = epScale;
			d0Scale0 = d0Scale;
			alphaScale0 = alphaScale;
			betaScale0 = betaScale;
		}

		if (densdepset) {
			if (indvarset) {
				if (d0Mean <= 0.0 || d0Mean > 1.0) {
					BatchError(filetype,line,20,"D0Mean");
					errors++;
				}
				if (d0SD <= 0.0 || d0SD > 1.0) {
					BatchError(filetype,line,20,"D0SD");
					errors++;
				}
				if (stage == 0 && sex == 0) {
					if (d0Scale <= 0.0 || d0Scale > 1.0) {
						BatchError(filetype,line,20,"D0Scale");
						errors++;
					}
				}
				if (d0SD > d0Scale0) {
					BatchError(filetype,line,3,"D0SD","D0Scale (first line)");
					errors++;
				}
				if (alphaSD <= 0.0) {
					BatchError(filetype,line,10,"alphaSD");
					errors++;
				}
				if (stage == 0 && sex == 0) {
					if (alphaScale0 <= 0.0) {
						BatchError(filetype,line,10,"alphaScale");
						errors++;
					}
				}
				if (alphaSD > alphaScale0) {
					BatchError(filetype,line,3,"alphaSD","alphaScale (first line)");
					errors++;
				}
				if (betaSD <= 0.0) {
					BatchError(filetype,line,10,"betaSD");
					errors++;
				}
				if (stage == 0 && sex == 0) {
					if (betaScale0 <= 0.0) {
						BatchError(filetype,line,10,"betaScale");
						errors++;
					}
				}
				if (betaSD > betaScale0) {
					BatchError(filetype,line,3,"betaSD","betaScale (first line)");
					errors++;
				}
			} else { // !indvarset
				if (d0 < 0.0 || d0 > 1.0) {
					BatchError(filetype,line,20,"D0");
					errors++;
				}
			...
			if (indvarset) {
				if (epMean <= 0.0 || epMean > 1.0) {
					BatchError(filetype,line,20,"EPMean");
					errors++;
				}
				if (epSD <= 0.0 || epSD > 1.0) {
					BatchError(filetype,line,20,"EPSD");
					errors++;
				}
				if (stage == 0 && sex == 0) {
					if (epScale <= 0.0 || epScale > 1.0) {
						BatchError(filetype,line,20,"EPScale");
						errors++;
					}
				}
				if (epSD > epScale0) {
					BatchError(filetype,line,3,"EPSD","EPScale (first line)");
					errors++;
				}
			} else { // !indvarset
				if (ep < 0.0 || ep > 1.0) {
					BatchError(filetype,line,20,"EP");
					errors++;
				}
			}
		}
...
// Parse header line;
	...
	case 0: { // negative exponential dispersal kernel
		...
		if (header != "SexDep" ) errors++;
		bTransferFile >> header;
		if (header != "KernelType" ) errors++;
		bTransferFile >> header;
		if (header != "DistMort" ) errors++;
		bTransferFile >> header;
		if (header != "IndVar" ) errors++;
		bTransferFile >> header;
		if (header != "Stage" ) errors++;
		...
		if (header != "Sex" ) errors++;
		bTransferFile >> header;
		if (header != "meanDistI" ) errors++;
		bTransferFile >> header;
		if (header != "meanDistII" ) errors++;
		bTransferFile >> header;
		if (header != "ProbKernelI" ) errors++;
		bTransferFile >> header;
		if (header != "DistIMean" ) errors++;
		bTransferFile >> header;
		if (header != "DistISD" ) errors++;
		bTransferFile >> header;
		if (header != "DistIIMean" ) errors++;
		bTransferFile >> header;
		if (header != "DistIISD" ) errors++;
		bTransferFile >> header;
		if (header != "ProbKernelIMean" ) errors++;
		bTransferFile >> header;
		if (header != "ProbKernelISD" ) errors++;
		bTransferFile >> header;
		if (header != "DistIScale" ) errors++;
		bTransferFile >> header;
		if (header != "DistIIScale" ) errors++;
		bTransferFile >> header;
		if (header != "ProbKernelIScale" ) errors++;
		bTransferFile >> header;
		if (header != "MortProb" ) errors++;
		bTransferFile >> header;
		if (header != "Slope" ) errors++;
		bTransferFile >> header;
		if (header != "InflPoint" ) errors++;
		break;
	...
	case 1: { // SMS
		...
		if (header != "PR" ) errors++;
		bTransferFile >> header;
		if (header != "PRMethod" ) errors++;
		bTransferFile >> header;
		if (header != "DP" ) errors++;
		bTransferFile >> header;
		if (header != "MemSize" ) errors++;
		bTransferFile >> header;
		if (header != "GB" ) errors++;
		bTransferFile >> header;
		if (header != "GoalType" ) errors++;
		bTransferFile >> header;
		if (header != "AlphaDB" ) errors++;
		bTransferFile >> header;
		if (header != "BetaDB" ) errors++;
		bTransferFile >> header;
		if (header != "DPMean" ) errors++;
		bTransferFile >> header;
		if (header != "DPSD" ) errors++;
		bTransferFile >> header;
		if (header != "GBMean" ) errors++;
		bTransferFile >> header;
		if (header != "GBSD" ) errors++;
		bTransferFile >> header;
		if (header != "AlphaDBMean" ) errors++;
		bTransferFile >> header;
		if (header != "AlphaDBSD" ) errors++;
		bTransferFile >> header;
		if (header != "BetaDBMean" ) errors++;
		bTransferFile >> header;
		if (header != "BetaDBSD" ) errors++;
		bTransferFile >> header;
		if (header != "DPScale" ) errors++;
		bTransferFile >> header;
		if (header != "GBScale" ) errors++;
		bTransferFile >> header;
		if (header != "AlphaDBScale" ) errors++;
		bTransferFile >> header;
		if (header != "BetaDBScale" ) errors++;
#if TEMPMORT
		bTransferFile >> header;
		...
		case 0: { // raster map with unique habitat codes
			...
			for (i = 0; i < maxNhab; i++) {
				...
				bTransferFile >> header;
				if (header != colheader ) morthaberrors++;
			}
			bTransferFile >> header;
			if (header != "CostMap" ) errors++;
			bTransferFile >> header;
			if (header != "CostMapFile" ) errors++;
			for (i = 0; i < maxNhab; i++) {
				colheader = "CostHab" + Int2Str(i+1);
				bTransferFile >> header;
				if (header != colheader ) costerrors++;
			}
			break;
		...
		case 9: { // artificial landscape
			...
			bTransferFile >> header;
			if (header != "MortHabitat" ) errors++;
			bTransferFile >> header;
			if (header != "MortMatrix" ) errors++;
			bTransferFile >> header;
			if (header != "CostMap" ) errors++;
			bTransferFile >> header;
			if (header != "CostMapFile" ) errors++;
			bTransferFile >> header;
			if (header != "CostHabitat" ) errors++;
			bTransferFile >> header;
			if (header != "CostMatrix" ) errors++;
			break;
		} // end of artificial landscape
	...
	case 2: { // CRW
		...
		if (header != "SL" ) errors++;
		bTransferFile >> header;
		if (header != "Rho" ) errors++;
		bTransferFile >> header;
		if (header != "StepLMean" ) errors++;
		bTransferFile >> header;
		if (header != "StepLSD" ) errors++;
		bTransferFile >> header;
		if (header != "RhoMean" ) errors++;
		bTransferFile >> header;
		if (header != "RhoSD" ) errors++;
		bTransferFile >> header;
		if (header != "StepLScale" ) errors++;
		bTransferFile >> header;
		if (header != "RhoScale" ) errors++;
		bTransferFile >> header;
		if (header != "StraightenPath" ) errors++;
		bTransferFile >> header;
		if (header != "SMtype" ) errors++;
	...
	case 3: { // 2Dt dispersal kernel
		...
		if (header != "DistMort" ) errors++;
		bTransferFile >> header;
		if (header != "U0Kernel1" ) errors++;
		bTransferFile >> header;
		if (header != "P0Kernel1" ) errors++;
		bTransferFile >> header;
		if (header != "U0Kernel2" ) errors++;
		bTransferFile >> header;
		if (header != "P0Kernel2" ) errors++;
		bTransferFile >> header;
		if (header != "PropKernel1" ) errors++;
		bTransferFile >> header;
		if (header != "MortProb" ) errors++;
		bTransferFile >> header;
		if (header != "Slope" ) errors++;
	...
	case 4: { // WALD dispersal kernel
		...
		if (header != "DistMort" ) errors++;
		bTransferFile >> header;
		if (header != "MeanU" ) errors++;
		bTransferFile >> header;
		if (header != "SigmaW" ) errors++;
		bTransferFile >> header;
		if (header != "Hc" ) errors++;
		for (i = 1; i < stages; i++) {
			colheader = "Hr" + Int2Str(i);
			bTransferFile >> header;
			if (header != colheader ) hrerrors++;
		}
		bTransferFile >> header;
		if (header != "Vt" ) errors++;
		bTransferFile >> header;
		if (header != "Kappa" ) errors++;
		bTransferFile >> header;
		if (header != "DirnMean" ) errors++;
		bTransferFile >> header;
		if (header != "DirnSD" ) errors++;
		bTransferFile >> header;
		if (header != "MortProb" ) errors++;
		bTransferFile >> header;
		if (header != "Slope" ) errors++;
	...
	while (simul != -98765) {
		...
		case 0: { // negative exponential dispersal kernel
			...
			if (kerneltype < 0 || kerneltype > 1) {
				BatchError(filetype,line,1,"KernelType");
				errors++;
			}
			// validate mortality
			...
			if (current.newsimul) {
				DistIScale0 = DistIScale;
				DistIIScale0 = DistIIScale;
			...
			if (!indvar) {
				if (meanDistI < resolution) {
					...
					BatchError(filetype,line,2,"meanDistI","Resolution");
					errors++;
				}
				if (kerneltype != 0) {
					if (meanDistII < resolution) {
						// NOTE - DITTO
						BatchError(filetype,line,2,"meanDistII","Resolution");
						errors++;
					}
					if (ProbKernelI <= 0.0 || ProbKernelI >= 1.0) {
						BatchError(filetype,line,20,"ProbKernelI");
						errors++;
					}
				}
			}
			...
			if (!stagestruct && indvar) {
				if (DistIMean < resolution) {
					// NOTE - DITTO
					BatchError(filetype,line,2,"DistIMean","Resolution");
					errors++;
				}
				if (DistISD <= 0.0) {
					BatchError(filetype,line,10,"DistISD");
					errors++;
				}
				if (DistISD > DistIScale0) {
					BatchError(filetype,line,3,"DistISD","DistIScale (first line)");
					errors++;
				}
				if (current.newsimul) {
					if (DistIScale <= 0.0) {
						BatchError(filetype,line,10,"DistIScale");
						errors++;
					}
				}
				if (kerneltype != 0) {
					if (DistIIMean < resolution) {
						// NOTE - DITTO
						BatchError(filetype,line,2,"DistIIMean","Resolution");
						errors++;
					}
					if (DistIISD <= 0.0) {
						BatchError(filetype,line,10,"DistIISD");
						errors++;
					}
					if (DistIISD > DistIIScale0) {
						BatchError(filetype,line,3,"DistIISD","DistIIScale (first line)");
						errors++;
					}
					if (current.newsimul) {
						if (DistIIScale <= 0.0) {
							BatchError(filetype,line,10,"DistIIScale");
							errors++;
						}
					}
					if (ProbKernelIMean <= 0.0 || ProbKernelIMean > 1.0) {
						BatchError(filetype,line,20,"ProbKernelIMean");
						errors++;
					}
					if (ProbKernelISD <= 0.0 || ProbKernelISD > 1.0) {
						BatchError(filetype,line,20,"ProbKernelISD");
						errors++;
					}
					if (ProbKernelISD > ProbKernelIScale0) {
						BatchError(filetype,line,3,"ProbKernelISD","ProbKernelIScale (first line)");
						errors++;
					}
					if (current.newsimul) {
						if (ProbKernelIScale <= 0.0 || ProbKernelIScale > 1.0) {
							BatchError(filetype,line,20,"ProbKernelIScale");
							errors++;
						}
					}
				}
		...
		case 1: { // SMS
			...
			if (pr < 1) {
				BatchError(filetype,line,11,"PR");
				errors++;
			}
			if (prMethod < 1 || prMethod > 3) {
				BatchError(filetype,line,33,"PRmethod");
				errors++;
			}
			if (!indvar && dp < 1.0) {
				BatchError(filetype,line,11,"DP");
				errors++;
			}
			if (memsize < 1 || memsize > 14) {
				BatchError(filetype,line,0,"MemSize");
				errors++;
				batchlog << "MemSize must be from 1 to 14" << endl;
			}
			if (!indvar && gb < 1.0) {
				BatchError(filetype,line,11,"GB");
				errors++;
			}
			if (goaltype < 0 || goaltype > 2) {
				BatchError(filetype,line,2,"GoalType");
				errors++;
			}
			if (!indvar && goaltype == 2) { // dispersal bias
				if (alphaDB <= 0.0) {
					BatchError(filetype,line,10,"AlphaDB");
					errors++;
				}
				if (betaDB <= 0.0) {
					BatchError(filetype,line,10,"BetaDB");
					errors++;
				}
			}
			...
			if (indvar) {
				if (dpMean < 1.0) {
					BatchError(filetype,line,11,"DPMean");
					errors++;
				}
				if (dpSD <= 0.0) {
					BatchError(filetype,line,10,"DPSD");
					errors++;
				}
				if (dpScale <= 0.0) {
					BatchError(filetype,line,10,"DPScale");
					errors++;
				}
				if (dpSD > dpScale) {
					BatchError(filetype,line,3,"DPSD","DPScale");
					errors++;
				}
				if (gbMean < 1.0) {
					BatchError(filetype,line,11,"GBMean");
					errors++;
				}
				if (gbSD <= 0.0) {
					BatchError(filetype,line,10,"GBSD");
					errors++;
				}
				if (gbScale <= 0.0) {
					BatchError(filetype,line,10,"GBScale");
					errors++;
				}
				if (gbSD > gbScale) {
					BatchError(filetype,line,3,"GBSD","GBScale");
					errors++;
				}
				if (goaltype == 2) { // dispersal bias
					if (alphaDBMean <= 0.0) {
						BatchError(filetype,line,10,"AlphaDBMean");
						errors++;
					}
					if (alphaDBSD <= 0.0) {
						BatchError(filetype,line,10,"AlphaDBSD");
						errors++;
					}
					if (alphaDBScale <= 0.0) {
						BatchError(filetype,line,10,"AlphaDBScale");
						errors++;
					}
					if (alphaDBSD > alphaDBScale) {
						BatchError(filetype,line,3,"AlphaDBSD","AlphaDBScale");
						errors++;
					}
					if (betaDBMean < 1.0) {
						BatchError(filetype,line,11,"BetaDBMean");
						errors++;
					}
					if (betaDBSD <= 0.0) {
						BatchError(filetype,line,10,"BetaDBSD");
						errors++;
					}
					if (betaDBScale <= 0.0) {
						BatchError(filetype,line,10,"BetaDBScale");
						errors++;
					}
					if (betaDBSD > betaDBScale) {
						BatchError(filetype,line,3,"BetaDBSD","BetaDBScale");
						errors++;
					}
				}
			}
			...
			if (straightenPath < 0 || straightenPath > 1) {
				BatchError(filetype,line,1,"StraightenPath");
				errors++;
			}
...
#if TEMPMORT
				if (smtype < 0 || smtype > 2) {
					BatchError(filetype,line,2,"SMtype");
					errors++;
				}
#else
				if (smtype < 0 || smtype > 1) {
					BatchError(filetype,line,1,"SMtype");
					errors++;
				}
#endif // TEMPMORT
			}
...
#endif // TEMPMORT 
			{
				if (smconst < 0.0 || smconst >= 1.0) {
					BatchError(filetype,line,20,"SMconst");
					errors++;
				}
			}
#if TEMPMORT
...
//			batchlog << "for LandType = 0" << endl;
				...
				if (costmap < 0 || costmap > 1) {
					BatchError(filetype,line,1,"CostMap");
					errors++;
				}
				ftype = "CostMapFile";
				if (costmap) {
				...
				for (i = 0; i < maxNhab; i++) {
					...
					if (!costmap) {
						if (costhab < 1) {
							colheader = "CostHab" + Int2Str(i+1);
							BatchError(filetype,line,11,colheader);
							errors++;
						}
					}
				}
...
//			batchlog << "for LandType = 9" << endl;
				...
				if (smtype) { // validate habitat-dependent mortality
					if (morthab < 0.0 || morthab >= 1.0) {
						BatchError(filetype,line,20,"MortHabitat");
						errors++;
					}
					if (mortmatrix < 0.0 || mortmatrix >= 1.0) {
						BatchError(filetype,line,20,"MortMatrix");
						errors++;
					}
				}
				if (costmap != 0) {
				...
				if (costhab < 1) {
					BatchError(filetype,line,11,"CostHabitat");
					errors++;
				}
				if (costmatrix < 1) {
					BatchError(filetype,line,11,"CostMatrix");
					errors++;
				}
				break;
			} // end of artificial landscape
		...
		case 2: { // CRW
			...
			if (indvar) { // individual variability
				if (StepLMean <= 0.0) {
					BatchError(filetype,line,10,"StepLMean");
					errors++;
				}
				if (StepLSD <= 0.0) {
					BatchError(filetype,line,10,"StepLSD");
					errors++;
				}
				if (StepLScale <= 0.0) {
					BatchError(filetype,line,10,"StepLScale");
					errors++;
				}
				if (StepLSD > StepLScale) {
					BatchError(filetype,line,3,"StepLSD","StepLScale");
					errors++;
				}
				if (RhoMean <= 0.0 || RhoMean >= 1.0) {
					BatchError(filetype,line,20,"RhoMean");
					errors++;
				}
				if (RhoSD <= 0.0 || RhoSD >= 1.0) {
					BatchError(filetype,line,20,"RhoSD");
					errors++;
				}
				if (RhoScale <= 0.0 || RhoScale >= 1.0) {
					BatchError(filetype,line,20,"RhoScale");
					errors++;
				}
				if (RhoSD > RhoScale) {
					BatchError(filetype,line,3,"RhoSD","RhoScale");
					errors++;
				}
			} else { // no individual variability
				if (SL <= 0.0) {
					BatchError(filetype,line,10,"SL");
					errors++;
				}
				if (rho <= 0.0 || rho >= 1.0) {
					BatchError(filetype,line,20,"Rho");
					errors++;
				}
			}
			if (straightenPath < 0 || straightenPath > 1) {
		...
		case 3: { // 2Dt dispersal kernel
			...
			if (u0Kernel1 < 0.0) {
				BatchError(filetype,line,19,"U0Kernel1");
				errors++;
			}
			if (p0Kernel1 < 0.0) {
				BatchError(filetype,line,19,"P0Kernel1");
				errors++;
			}
			if (u0Kernel2 < 0.0) {
				BatchError(filetype,line,19,"U0Kernel2");
				errors++;
			}
			if (p0Kernel2 < 0.0) {
				BatchError(filetype,line,19,"P0Kernel2");
				errors++;
			}
			if (propKernel1 < 0.0 || propKernel1 > 1.0) {
				BatchError(filetype,line,20,"PropKernel1");
				errors++;
			}
...
int ParseSettleFile(void)
{
	...
	float s0scale0,alphaSscale0,betaSscale0;
	int errors = 0;
	int simuls = 0;
	string filetype = "SettlementFile";

// Parse header line;
	bSettlementFile >> header;
	if (header != "Simulation" ) errors++;
	bSettlementFile >> header;
	if (header != "StageDep" ) errors++;
	bSettlementFile >> header;
...
#endif // RS_CONTAIN 
	{
		// dispersal kernel
		bSettlementFile >> header;
		if (header != "SettleType" ) errors++;
		bSettlementFile >> header;
		if (header != "FindMate" ) errors++;
		...
		if (header != "FindMate" ) errors++;
		bSettlementFile >> header;
		if (header != "MinSteps" ) errors++;
		bSettlementFile >> header;
		if (header != "MaxSteps" ) errors++;
		bSettlementFile >> header;
		if (header != "MaxStepsYear" ) errors++;
#if GOBYMODEL
		bSettlementFile >> header;
		if (header != "AlphaSasocial" ) errors++;
		bSettlementFile >> header;
		if (header != "BetaSasocial" ) errors++;
#endif
		bSettlementFile >> header;
		if (header != "S0" ) errors++;
		bSettlementFile >> header;
		if (header != "AlphaS" ) errors++;
		bSettlementFile >> header;
		if (header != "BetaS" ) errors++;
		bSettlementFile >> header;
		if (header != "S0Mean" ) errors++;
		bSettlementFile >> header;
		if (header != "S0SD" ) errors++;
		bSettlementFile >> header;
		if (header != "AlphaSMean" ) errors++;
		bSettlementFile >> header;
		if (header != "AlphaSSD" ) errors++;
		bSettlementFile >> header;
		if (header != "BetaSMean" ) errors++;
		bSettlementFile >> header;
		if (header != "BetaSSD" ) errors++;
		bSettlementFile >> header;
		if (header != "S0Scale" ) errors++;
		bSettlementFile >> header;
		if (header != "AlphaSScale" ) errors++;
		bSettlementFile >> header;
		if (header != "BetaSScale" ) errors++;
	}
	if (errors > 0) {
		FormatError(filetype,errors);
		return -111;
	...
	while (simul != -98765) {
...
#endif // RS_CONTAIN 
		{
			...
			if (settletype < 0 || settletype > 3) {
				BatchError(filetype,line,3,"SettleType");
				errors++;
			}
			...
			if (sexesDisp > 1) {
				if (findmate < 0 || findmate > 1) {
					BatchError(filetype,line,1,"FindMate");
					errors++;
				}
			}
		} else { // movement method
			...
			if (stage == 0 && sex == 0) {
				if (minSteps < 0) {
					BatchError(filetype,line,19,"MinSteps");
					errors++;
				}
				if (maxSteps < 0) {
					BatchError(filetype,line,19,"MaxSteps");
					errors++;
				}
			}
			if (maxStepsYear < 0) {
				BatchError(filetype,line,19,"MaxStepsYear");
				errors++;
			}
#if GOBYMODEL
			...
			if (densdep == 1 && stage == 0 && sex == 0) {
				if (betaSasoc <= 0.0) {
					BatchError(filetype,line,10,"BetaSasocial");
					errors++;
				}
			}
#endif
...
//	<< endl;
			...
			if (current.newsimul) {
				// record scaling factors from first line of the simulation
				s0scale0 = s0scale;
				alphaSscale0 = alphaSscale;
				betaSscale0 = betaSscale;
			}
...
//DEBUGLOG << "ParseSettleFile(): validating initial trait parameters" << endl;
						...
						if (s0mean <= 0.0 || s0mean > 1.0) {
							BatchError(filetype,line,20,"S0Mean");
							errors++;
						}
						if (s0sd <= 0.0 || s0sd > 1.0) {
							BatchError(filetype,line,20,"S0SD");
							errors++;
						}
						if (alphaSsd <= 0.0) {
							BatchError(filetype,line,10,"AlphaSsd");
							errors++;
						}
						if (betaSsd <= 0.0) {
							BatchError(filetype,line,10,"BetaSsd");
							errors++;
						}
						if (sex == 0) {
							if (s0scale <= 0.0 || s0scale > 1.0) {
								BatchError(filetype,line,20,"S0Scale");
								errors++;
							}
							if (alphaSscale <= 0.0) {
								BatchError(filetype,line,10,"AlphaSscale");
								errors++;
							}
							if (betaSscale <= 0.0) {
								BatchError(filetype,line,10,"BetaSscale");
								errors++;
							}
						}
						if (s0sd > s0scale0) {
							BatchError(filetype,line,3,"S0SD","S0Scale (first line)");
							errors++;
						}
						if (alphaSsd > alphaSscale0) {
							BatchError(filetype,line,3,"AlphaSsd","AlphaSscale (first line)");
							errors++;
						}
						if (betaSsd > betaSscale0) {
							BatchError(filetype,line,3,"BetaSsd","BetaSscale (first line)");
							errors++;
						}
					}
...
#endif
					if (s0 <= 0.0 || s0 > 1.0) {
						BatchError(filetype,line,20,"S0");
						errors++;
					}
...
// Parse header line;
	bGeneticsFile >> header;
	if (header != "Simulation" ) errors++;
	bGeneticsFile >> header;
	if (header != "Architecture" ) errors++;
	bGeneticsFile >> header;
	if (header != "NLoci" ) errors++;
	bGeneticsFile >> header;
	if (header != "ArchFile" ) errors++;
	bGeneticsFile >> header;
	if (header != "ProbMutn" ) errors++;
	bGeneticsFile >> header;
	if (header != "ProbCross" ) errors++;
	bGeneticsFile >> header;
	if (header != "AlleleSD" ) errors++;
	bGeneticsFile >> header;
	if (header != "MutationSD" ) errors++;
	if (errors > 0) {
		FormatError(filetype,errors);
		return -111;
	}
...
// first simulation number must match first one in parameterFile
	...
	while (simul != -98765) {
		...
		if (arch < 0 || arch > 1) {
			BatchError(filetype,line,1,"Architecture");
			errors++;
		}
		...
		if (arch == 0) {
			if (nLoci < 1) {
				BatchError(filetype,line,11,"NLoci");
				errors++;
			}
		}
		if (probMutn < 0.0 || probMutn > 1.0) {
			BatchError(filetype,line,20,"ProbMutn");
			errors++;
		}
		if (probCross < 0.0 || probCross > 1.0) {
			BatchError(filetype,line,20,"ProbCross");
			errors++;
		}
		if (alleleSD <= 0.0) {
			BatchError(filetype,line,10,"AlleleSD");
			errors++;
		}
		if (mutationSD <= 0.0) {
			BatchError(filetype,line,10,"MutationSD");
			errors++;
		}
...
// check no. of chromosomes, and terminate if in error
	...
	if (paramname == "NChromosomes") {
		if (nchromosomes < 1) {
			BatchError(filetype,-999,11,"NChromosomes");
			errors++;
			return -111;
		}
	} else {
...
//batchlog << "paramname=" << paramname << " (after loop)" << endl;
	...
	if (lociError) {
		BatchError(filetype,-999,11,"Trait NLoci");
		errors++;
	}
	if (chromError) {
		BatchError(filetype,-999,0," ");
...
// Parse header line;
	...
	if (header != "Simulation" ) errors++;
	bManageFile >> header;
	if (header != "Method" ) errors++;
	bManageFile >> header;
	if (header != "Timing" ) errors++;
	bManageFile >> header;
	if (header != "DistDecay" ) errors++;
	bManageFile >> header;
	if (header != "MaxPatches" ) errors++;
//bManageFile >> header; if (header != "ThresholdPop" ) errors++;
	bManageFile >> header;
	if (header != "ThresholdDens" ) errors++;
	bManageFile >> header;
	if (header != "CountCV" ) errors++;
	if (stagestruct) {
		bManageFile >> header;
		if (header != "CullNstages" ) errors++;
		bManageFile >> header;
		if (header != "CullStage" ) errors++;
	}
	bManageFile >> header;
	if (header != "CullRate" ) errors++;
	bManageFile >> header;
	if (header != "CullMaxRate" ) errors++;
	bManageFile >> header;
	if (header != "CullAlpha" ) errors++;
	bManageFile >> header;
	if (header != "CullBeta" ) errors++;
...
//}
	bManageFile >> header;
	if (header != "DamageTiming" ) errors++;
	bManageFile >> header;
	if (header != "DamageType" ) errors++;
	bManageFile >> header;
	if (header != "OccOption" ) errors++;
	bManageFile >> header;
	if (header != "DamageStage" ) errors++;
	bManageFile >> header;
	if (header != "AlphaOccupancy" ) errors++;
	bManageFile >> header;
	if (header != "BetaOccupancy" ) errors++;
	bManageFile >> header;
	if (header != "AlphaTraversal" ) errors++;
	bManageFile >> header;
	if (header != "BetaTraversal" ) errors++;
// report any errors in headers, and if so, terminate validation
...
// first simulation number must match first one in parameterFile
	...
	while (simul != -98765) {
		...
		if (current.newsimul && (inint < 0 || inint > 7)) {
			BatchError(filetype,line,7,"Method");
			errors++;
		}
		bManageFile >> inint;
		if (current.newsimul && (inint < 0 || inint > 1)) {
			BatchError(filetype,line,1,"Timing");
			errors++;
		}
		bManageFile >> infloat;
		if (current.newsimul && infloat < 0.0) {
			BatchError(filetype,line,10,"DistDecay");
			errors++;
		}
		bManageFile >> inint;
		if (current.newsimul && inint < 2) {
			BatchError(filetype,line,12,"MaxPatches");
			errors++;
		}
//	bManageFile >> inint;
//	if (inint < 1) {
...
//	}
		bManageFile >> infloat;
		if (current.newsimul && infloat <= 0.0) {
			BatchError(filetype,line,10,"ThresholdDens");
			errors++;
		}
		bManageFile >> infloat;
		if (current.newsimul && infloat < 0.0) {
			BatchError(filetype,line,19,"CountCV");
			errors++;
		}
		if (stagestruct) {
			...
			prevstage = cullstage;
			prevNstages = cullNstages;
			nstages++;
		}
		bManageFile >> cullrate;
		if (stagestruct) {
			if (cullrate < 0 || cullrate > 2) {
				BatchError(filetype,line,2,"CullRate");
				errors++;
			}
			if (current.newsimul) {
			...
			if (cullrate < 0 || cullrate > 1) {
				BatchError(filetype,line,1,"CullRate");
				errors++;
			}
		}
		bManageFile >> infloat;
		if (current.newsimul || cullrate == 2) {
			if (infloat < 0.0 || infloat > 1.0) {
				BatchError(filetype,line,20,"CullMaxRate");
				errors++;
			}
		}
...
//	}
		...
		if (damagetype == 2 && infloat < 0.0) {
			BatchError(filetype,line,10,"AlphaOccupancy");
			errors++;
		}
		bManageFile >> infloat;
		if (damagetype == 1 && infloat < 0.0) {
			BatchError(filetype,line,10,"BetaOccupancy");
			errors++;
		}
		bManageFile >> infloat;
		if (transfer == 1 && damagetype == 2 && infloat < 0.0) {
			BatchError(filetype,line,10,"AlphaTraversal");
			errors++;
		}
		bManageFile >> infloat;
		if (transfer == 1 && damagetype == 1 && infloat < 0.0) {
			BatchError(filetype,line,10,"BetaTraversal");
			errors++;
		}
...
int ParseInitFile(string indir)
{
	string header,colheader;
	int i,simul;
	int seedtype,freetype,sptype,initdens,indscell,minX,maxX,minY,maxY;
...
// Parse header line;
	...
	if (header != "Simulation" ) errors++;
	bInitFile >> header;
	if (header != "SeedType" ) errors++;
	bInitFile >> header;
	if (header != "FreeType" ) errors++;
	bInitFile >> header;
	if (header != "SpType" ) errors++;
	bInitFile >> header;
	if (header != "InitDens" ) errors++;
	bInitFile >> header;
	if (patchmodel) {
		if (header != "IndsHa" ) errors++;
	} else 						{
		if (header != "IndsCell" ) errors++;
	}
	bInitFile >> header;
	if (header != "minX" ) errors++;
	bInitFile >> header;
	if (header != "maxX" ) errors++;
	bInitFile >> header;
	if (header != "minY" ) errors++;
	bInitFile >> header;
	if (header != "maxY" ) errors++;
	bInitFile >> header;
	if (header != "NCells" ) errors++;
	bInitFile >> header;
	if (header != "NSpCells" ) errors++;
	bInitFile >> header;
	if (header != "InitFreezeYear" ) errors++;
	bInitFile >> header;
	if (header != "RestrictRows" ) errors++;
	bInitFile >> header;
	if (header != "RestrictFreq" ) errors++;
	bInitFile >> header;
	if (header != "FinalFreezeYear" ) errors++;
	bInitFile >> header;
	if (header != "InitIndsFile" ) errors++;
	if (stagestruct) {
		bInitFile >> header;
		if (header != "InitAge" ) errors++;
		for (i = 1; i < stages; i++) {
			colheader = "PropStage" + Int2Str(i);
			bInitFile >> header;
			if (header != colheader ) propnerrors++;
		}
	}
// report any errors in headers, and if so, terminate validation
...
// first simulation number must match first one in parameterFile
	...
	while (simul != -98765) {
		...
		if (seedtype < 0 || seedtype > 2) {
			BatchError(filetype,line,2,"SeedType");
			errors++;
		}
		...
		if (seedtype == 0) {
			if (freetype < 0 || freetype > 1) {
				BatchError(filetype,line,1,"FreeType");
				errors++;
			}
		}
		if (seedtype == 1) {
			if (sptype < 0 || sptype > 1) {
				BatchError(filetype,line,1,"SpType");
				errors++;
			}
		}
		if (initdens < 0 || initdens > 2) {
			BatchError(filetype,line,2,"initDens");
			errors++;
		}
		if (seedtype < 2) {
			if (initdens == 2) { // specified density
				if (patchmodel) {
					if (inds_per_ha <= 0.0) {
						BatchError(filetype,line,10,"IndsHa");
						errors++;
					}
				} else {
					if (indscell < 1) {
						BatchError(filetype,line,11,"IndsCell");
						errors++;
					}
				}
			}
...
//		}
			if (maxX < minX) {
				BatchError(filetype,line,2,"maxX","minX");
				errors++;
			}
//		if (minY < 0) {
...
//		}
			if (maxY < minY) {
				BatchError(filetype,line,2,"maxY","minY");
				errors++;
			}
		}
		if (seedtype == 0 && freetype == 0) {
			if (nCells < 1) {
				BatchError(filetype,line,11,"NCells");
				errors++;
			}
			int range_cells;
		...
		if (seedtype == 1 && sptype == 1 && nSpCells < 1) {
			BatchError(filetype,line,11,"NSpCells");
			errors++;
		}
		...
		if (seedtype == 0) {
			if (initFreezeYear < 0) {
				BatchError(filetype,line,19,"InitFreezeYear");
				errors++;
			}
			if (restrictRows < 0) {
				BatchError(filetype,line,19,"RestrictRows");
				errors++;
			}
			if (restrictRows > 0 && restrictFreq <= 0) {
				BatchError(filetype,line,10,"RestrictFreq");
				errors++;
			}
			if (finalFreezeYear < 0) {
				BatchError(filetype,line,19,"FinalFreezeYear");
				errors++;
			} else {
				if (finalFreezeYear > 0 && finalFreezeYear <= initFreezeYear) {
					BatchError(filetype,line,1,"FinalFreezeYear","InitFreezeYear");
					errors++;
				}
			}
		}
...
// Parse header line;
	bVirtEcolFile >> header;
	if (header != "Simulation" ) errors++;
	bVirtEcolFile >> header;
	if (header != "PatchMethod" ) errors++;
	bVirtEcolFile >> header;
	if (header != "RowsFront" ) errors++;
	bVirtEcolFile >> header;
	if (header != "MaxNPatches" ) errors++;
	bVirtEcolFile >> header;
	if (header != "MaxPatchNum" ) errors++;
	bVirtEcolFile >> header;
	if (header != "PatchFile" ) errors++;
	bVirtEcolFile >> header;
	if (header != "MinIndsPatch" ) errors++;
	bVirtEcolFile >> header;
	if (header != "MaxIndsPatch" ) errors++;
	bVirtEcolFile >> header;
	if (header != "StageMethod" ) errors++;
	bVirtEcolFile >> header;
	if (header != "MinX" ) errors++;
	bVirtEcolFile >> header;
	if (header != "MaxX" ) errors++;
	...
	if (header != "MaxY" ) errors++;
	bVirtEcolFile >> header;
	if (header != "LandGen" ) errors++;
	bVirtEcolFile >> header;
	if (header != "LandGenStart" ) errors++;
	bVirtEcolFile >> header;
	if (header != "LandGenFreq" ) errors++;
	bVirtEcolFile >> header;
	if (header != "WriteSamples" ) errors++;
	bVirtEcolFile >> header;
	if (header != "SampleLoci" ) errors++;
	bVirtEcolFile >> header;
	if (header != "SampleFile" ) errors++;
	if (errors > 0) {
		FormatError(filetype,errors);
		return -111;
	}
...
// first simulation number must match first one in parameterFile
	...
	while (simul != -98765) {
		...
		if (patchMethod < 0 || patchMethod > 3) {
			BatchError(filetype,line,3,"PatchMethod");
			errors++;
		}
		if (patchMethod == 2) {
			if (rowsFront < 1) {
				BatchError(filetype,line,11,"RowsFront");
				errors++;
			}
		}
		if (maxNPatches < 2) {
			BatchError(filetype,line,12,"MaxNPatches");
			errors++;
		}
		if (maxPatchNum < 0) {
			BatchError(filetype,line,19,"MaxPatchNum");
			errors++;
		}
		...
		if (patchMethod == 2) {
			if (minIndsPatch < 2) {
				BatchError(filetype,line,12,"MinIndsPatch");
				errors++;
			}
		}
		if (maxIndsPatch < 2) {
			BatchError(filetype,line,12,"MaxIndsPatch");
			errors++;
		}
		if (stagestruct) {
			if (stageMethod < 0 || stageMethod > 2) {
				BatchError(filetype,line,2,"StageMethod");
				errors++;
			}
		}
		if (patchMethod == 1) {
			if (minX < 0) {
				BatchError(filetype,line,19,"MinX");
				errors++;
			}
			if (maxX < minX) {
				BatchError(filetype,line,2,"MaxX","MinX");
			...
			if (minY < 0) {
				BatchError(filetype,line,19,"MinY");
				errors++;
			}
			if (maxY < minY) {
				BatchError(filetype,line,2,"MaxY","MinY");
		...
		if (landGen < 0 || landGen > 1) {
			BatchError(filetype,line,1,"LandGen");
			errors++;
		}
		if (landGen == 1) {
			if (landGenStart < 0) {
				BatchError(filetype,line,19,"LandGenStart");
				errors++;
			}
			if (landGenFreq < 0) {
				BatchError(filetype,line,19,"LandGenFreq");
				errors++;
			}
			if (writeSamples < 0 || writeSamples > 1) {
				BatchError(filetype,line,1,"WriteSamples");
				errors++;
			}
			if (sampleLoci < 0 || sampleLoci > 1) {
				BatchError(filetype,line,1,"SampleLoci");
				errors++;
			}
		}
...
// check no. of samples, and terminate if in error
	...
	if (paramname == "NSamples") {
		if (nsamples < 2) {
			BatchError(filetype,-999,11,"NSamples");
			errors++;
			return -111;
		}
	} else {
...
// Parse header line;
	bABCpFile >> header;
	if (header != "ParamNum" ) errors++;
	bABCpFile >> header;
	if (header != "ParamType" ) errors++;
	bABCpFile >> header;
	if (header != "ParamName" ) errors++;
	bABCpFile >> header;
	if (header != "Stage" ) errors++;
	bABCpFile >> header;
	if (header != "Sex" ) errors++;
	bABCpFile >> header;
	if (header != "PriorType" ) errors++;
	bABCpFile >> header;
	if (header != "Pr0" ) errors++;
	bABCpFile >> header;
	if (header != "Pr1" ) errors++;
	if (errors > 0) {
		FormatError(filetype,errors);
		batchlog << "*** Ensure column headers are correct to continue checking data" << endl;
		return -111;
...
// Parse data lines
	...
	while (inint != -98765) {
		bABCpFile >> type;
		if (type < 1 || type > 5) {
			BatchError(filetype,line,55,"ParamType");
			errors++;
		}
		nm = "ZZZxxxZZZxxx";
		bABCpFile >> nm;
		...
		bABCpFile >> stage >> sex;
		bABCpFile >> priortype;
		if (priortype < 1 || priortype > 6) {
			BatchError(filetype,line,66,"PriorType");
			errors++;
		}
		bABCpFile >> pr0 >> pr1;
		...
		case 1: // uniform
			if (pr0 >= pr1) {
				BatchError(filetype,line,1,"Pr1","Pr0");
				errors++;
			}
			break;
		case 2: // normal
			if (pr1 <= 0.0) { // s.d.
				BatchError(filetype,line,10,"Pr1");
				errors++;
			}
			break;
		case 3: // log-normal
			if (pr0 <= 0.0) { // mean
				BatchError(filetype,line,10,"Pr0");
				errors++;
			}
			if (pr1 <= 1.0) { // s.d.
				BatchError(filetype,line,21,"Pr1");
				errors++;
			}
			break;
		case 4: // beta
...
// Parse header line;
	bABCoFile >> header;
	if (header != "ObsNum" ) errors++;
	bABCoFile >> header;
	if (header != "ObsType" ) errors++;
	bABCoFile >> header;
	if (header != "ObsName" ) errors++;
	bABCoFile >> header;
	if (header != "Year" ) errors++;
	bABCoFile >> header;
	if (header != "Loc0" ) errors++;
	bABCoFile >> header;
	if (header != "Loc1" ) errors++;
	bABCoFile >> header;
	if (header != "Stage" ) errors++;
	bABCoFile >> header;
	if (header != "Sex" ) errors++;
	bABCoFile >> header;
	if (header != "Value" ) errors++;
	bABCoFile >> header;
	if (header != "Weight" ) errors++;
	if (errors > 0) {
		FormatError(filetype,errors);
		batchlog << "*** Ensure column headers are correct to continue checking data" << endl;
		return -111;
...
// Parse data lines
	...
	while (inint != -98765) {
		bABCoFile >> type;
		if (type < 1 || type > 5) {
			BatchError(filetype,line,55,"ObsType");
			errors++;
		}
		nm = "ZZZxxxZZZxxx";
		bABCoFile >> nm;
		...
		if (year < 1) {
			BatchError(filetype,line,11,"Year");
			errors++;
		}
		switch (type) {
		case 1: // range
		...
		case 2: // population
			...
			if (nm == "NInds") {
				...
				if (reproductn == 0) {
					...
					if (sex < 0 || sex > 2) {
						BatchError(filetype,line,2,"Sex");
						errors++;
					}
				}
			}
			if (nm == "Occupied") {
				if (value != 0.0 && value != 1.0) {
					BatchError(filetype,line,1,"Value");
					errors++;
				}
			}
			break;
		case 3: // individual
			if (nm == "EmigRate" || nm == "DispSuccess") {
				if (value < 0.0 || value > 1.0) {
					BatchError(filetype,line,20,"Value");
					errors++;
				}
			}
			break;
		...
		if (nm == "NInds" || nm == "NOccupPatches" || nm == "DistMean" || nm == "DistSD") {
			if (value <= 0.0) {
				BatchError(filetype,line,10,"Value");
				errors++;
			}
		}
		if (weight <= 0) {
			BatchError(filetype,line,10,"Weight");
			errors++;
		}
		line++;
		// read next simulation number
...
// validate stagedep
	if (stagestruct) {
		if (stagedep < 0 || stagedep > 1) {
			BatchError(filetype,line,1,"StageDep");
			current.errors++;
...
// validate sexdep
	if (sexesDisp == 2) {
		if (sexdep < 0 || sexdep > 1) {
			BatchError(filetype,line,1,"SexDep");
			current.errors++;
...
// validate stage
	if (stagedep) { // there must be 1 or 2 lines for each stage
		if (sexdep) { // there must be 2 lines for each stage
			if (current.simlines%2) iii = (current.simlines+1)/2;
			else  iii = current.simlines/2;
			if (stage != iii-1) {
				BatchError(filetype,line,0," ");
				current.errors++;
...
// validate indvar
	if (stagedep && !stgdepindvarok) {
		...
		if (indvar < 0 || indvar > 1) {
			BatchError(filetype,line,1,"IndVar");
			current.errors++;
		}
	}
...
#endif
	parameters >> iiii;
	if (iiii == 1) sim.absorbing = true;
	else sim.absorbing = false;
#if RSDEBUG
//DEBUGLOG << "ReadParameters(): paramsSim=" << paramsSim << endl;
	...
	if (iiii == 1 && gradType != 0) shifting = true;
	else shifting = false;
	...
	if (iiii == 0) env.stoch = false;
		...
		env.stoch = true;
		if (iiii == 2) env.local = true;
		else env.local = false;
	}
	if (paramsLand.patchModel && env.local) error = 101;
	parameters >> iiii;
	if (iiii == 1) env.inK = true;
	else env.inK = false;
#if BUTTERFLYDISP
	parameters >> iiii;
...
//parameters >> env.ac >> env.std >> env.minR >> env.maxR >> env.minK >> env.maxK;
	...
	if (env.inK) {
		float minKK,maxKK;
		minKK = minK * (((float)(paramsLand.resol*paramsLand.resol))/10000.0);
		maxKK = maxK * (((float)(paramsLand.resol*paramsLand.resol))/10000.0);
...
#endif
	parameters >> iiii;
	if (iiii == 1) env.localExt = true;
	else env.localExt = false;
...
#endif // RS_CONTAIN 
	if (sim.outIntRange > 0)     sim.outRange = true;
	else sim.outRange = false;
	if (sim.outIntOcc > 0)       sim.outOccup = true;
	else sim.outOccup = false;
	if (sim.outIntPop > 0)       sim.outPop = true;
	else sim.outPop = false;
	if (sim.outIntInd > 0)       sim.outInds = true;
	else sim.outInds = false;
	if (sim.outIntGenetic > 0)   sim.outGenetics = true;
	else sim.outGenetics = false;
	if (jjjj == 1)   						 sim.outGenXtab = true;
	else sim.outGenXtab = false;
	if (sim.outIntRange > 0)     sim.outRange = true;
	else sim.outRange = false;
	if (sim.outIntTraitCell > 0) sim.outTraitsCells = true;
	else sim.outTraitsCells = false;
	if (sim.outIntTraitRow > 0)  sim.outTraitsRows = true;
	else sim.outTraitsRows = false;
	if (sim.outIntConn > 0)      sim.outConnect = true;
	else sim.outConnect = false;
#if RS_CONTAIN
	if (sim.outIntDamage > 0)    sim.outDamage = true;
...
//sim.saveInitMap = false;
	if (iiii == 0) sim.drawLoaded = false;
	else sim.drawLoaded = true;
...
// 'name' is TransMatrixFile
	...
	for (short i = 1; i < 7; i++) {
		ssfile >> prop;
		pSpecies->setPropDispMigrn(i,prop);
	}
	ssfile >> resetmigrn;
...
int ReadEmigration(int option)
{
	...
	int Nlines,simulation,firstsimul,stage,sex,emigstage;
#if GOBYMODEL
	float asocD;
#endif
	float	ep,d0,alpha,beta,epMean,epSD,d0Mean,d0SD,alphaMean,alphaSD,betaMean,betaSD;
...
//	<< " sstruct.nStages = " << sstruct.nStages << " sexesDisp = " << sexesDisp << endl;
	...
	for (int line = 0; line < Nlines; line++) {
		...
		if (firstline) {
			...
			if (iiii == 0) emig.densDep = false;
			else emig.densDep = true;
			if (jjjj == 0) emig.stgDep = false;
			else emig.stgDep = true;
			if (kkkk == 0) emig.sexDep = false;
			else emig.sexDep = true;
			if (llll == 0) emig.indVar = false;
			...
			if (ffff == 0) pSpecies->setFullKernel(false);
...
//---------------------------------------------------------------------------
		...
		if (emig.sexDep) {
			if (emig.stgDep) {
				...
				if (emig.indVar) {
					if (emig.densDep) {
						eparams.d0Mean = d0Mean;
						eparams.d0SD = d0SD;
						eparams.alphaMean = alphaMean;
						eparams.alphaSD = alphaSD;
						eparams.betaMean = betaMean;
						eparams.betaSD = betaSD;
					} else {
						eparams.d0Mean = epMean;
						eparams.d0SD = epSD;
...
//DEBUGLOG << "ReadEmigration(): case 0: emigP = " << ep << endl;
		...
		if (stage == 0 && sex == 0) {
			...
			if (emig.densDep) {
				scale.d0Scale = d0Scale;
				scale.alphaScale = alphaScale;
				scale.betaScale = betaScale;
			} else {
				scale.d0Scale = epScale;
				scale.alphaScale = scale.betaScale = 0.000001;
			}
...
{
	int iiii,jjjj,kkkk,Nlines,simulation,firstsimul,stageDep,sexDep,stage,sex;
	float tttt;
	bool firstline = true;
	int error = 0;
	landParams paramsLand = pLandscape->getLandParams();
	...
	case 0: // negative exponential dispersal kernel
		...
		for (int line = 0; line < Nlines; line++) {
			...
			if (firstline) {
				firstsimul = simulation;
#if !RS_CONTAIN
				if (iiii == 0) trfr.twinKern = false;
				else trfr.twinKern = true;
#endif // RS_CONTAIN 
				if (jjjj == 0) trfr.distMort = false;
				else trfr.distMort = true;
				sexKernels = 2*stageDep + sexDep;
				if (kkkk == 0) trfr.indVar = false;
				else trfr.indVar = true;
				if (sexDep) trfr.sexDep = true;
...
//				MAXDist = kparams.maxDist1;
					pSpecies->setKernParams(0,0,kparams,paramsLand.resol);
				} else {
...
//			if (dem.stageStruct) error = 412;
	...
	case 1: // SMS
		...
		if (iiii == 0) trfr.habMort = false;
		else trfr.habMort = true;
#endif // TEMPMORT 
		if (jjjj == 0) move.straigtenPath = false;
		else move.straigtenPath = true;
...
//	<< " i = " << i << " mortality = " << tttt << endl;
...
#endif // TEMPMORT 
			{
				...
				transFile >> tttt;
				pSpecies->setHabMort(1,tttt);
				transFile >> tttt;
				pSpecies->setHabMort(0,tttt);
#if RSDEBUG
		...
		if (iiii == 0) trfr.costMap = false;
		else trfr.costMap = true;
#if RSDEBUG
...
int ReadSettlement(int option)
{
	...
	int Nlines,simulation,firstsimul,stageDep,sexDep,stage,sex;
	bool firstline = true;
	int error = 0;
	demogrParams dem = pSpecies->getDemogr();
	stageParams sstruct = pSpecies->getStage();
	...
	settParams sparams;
	int sexSettle,settType,densdep,indvar,findmate;
#if GOBYMODEL
	float alphaSasoc,betaSasoc;
#endif
...
// set no.of lines assuming maximum stage- and sex-dependency
	...
	for (int line = 0; line < Nlines; line++) {
		...
		if (firstline) {
			...
			if (stageDep == 1) sett.stgDep = true;
			else sett.stgDep = false;
			if (sexDep == 1) sett.sexDep = true;
			else sett.sexDep = false;
#if RS_CONTAIN
			if (transfer == 0 || transfer == 3 || transfer == 4) sett.indVar = false;  // dispersal kernel
#else
			if (transfer == 0) sett.indVar = false;  // dispersal kernel
#endif // RS_CONTAIN 
			else if (indvar == 1) sett.indVar = true;
			else sett.indVar = false;
		...
		if (trfr.moveModel) {
			...
			case 0: // no sex- / stage-dependence
				...
				if (densdep == 1) srules.densDep = true;
				else srules.densDep = false;
				if (findmate == 1) srules.findMate = true;
				else srules.findMate = false;
...
//	if (dem.stageStruct) nheaders += sstruct.nStages;
	...
	for (int i = 0; i < Nlines; i++) {
		...
		if (c.cullRate == 2) {
			cstage.cullMaxRate = cullmaxrate;
			cstage.cullAlpha = cullalpha;
			cstage.cullBeta = cullbeta;
		} else {
			c.cullMaxRate = cullmaxrate;
			c.cullAlpha = cullalpha;
			c.cullBeta = cullbeta;
		}
//	amfile >> iiii;
...
int ReadVirtEcol(int option)
{
	...
	if (landgen == 1) virt.landscapeGenetics = true;
	else virt.landscapeGenetics = false;
	if (outgen == 1) virt.outGenomes = true;
	else virt.outGenomes = false;
	if (sampleloci == 1) pSpecies->setSampleAllLoci(false);
	else pSpecies->setSampleAllLoci(true);
...
//	<< " parameters.open() = " << pppp << endl;
			...
			for (int i = 0; i < nSimuls; i++) {
				...
				if (geneticsFile == "NULL") {
					...
					if (reproductn == 0) g.diploid = false;
					else g.diploid = true;
