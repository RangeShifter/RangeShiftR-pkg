...
double LnFac(int32 n) {
  ...
  if (n < FAK_LEN) {
    if (n <= 1) {
//	  if (n < 0) FatalError(L"Parameter negative in LnFac function");
	  return 0;}
...
*/
  ...
  if (L < 17) {
    if (L < 1.E-6) {
      if (L == 0) return 0;
//	  if (L < 0) FatalError(L"Parameter negative in poisson function");
...
//	if (L > 2.E9) FatalError(L"Parameter too big in poisson function");
...
//	FatalError(L"Parameter out of range in binomial function");
    } // error exit
...
//  if (n > N || m > N || n < 0 || m < 0) {
//	FatalError(L"Parameter out of range in hypergeometric function");}
...
//  if (p < 0 || p > 1) FatalError(L"Parameter out of range in Bernoulli function");
  return Random() < p;}
...
*/
  ...
  int32 x;
  int i;
//  if (n < 0 || colors < 0) FatalError(L"Parameter negative in multinomial function");
  if (colors == 0) return;
  
  // compute sum of probabilities
  for (i=0, sum=0; i<colors; i++) { 
    s = source[i];
//	if (s < 0) FatalError(L"Parameter negative in multinomial function");
    sum += s;}
//  if (sum == 0 && n > 0) FatalError(L"Zero sum in multinomial function");
...
//  if (n < 0 || colors < 0) FatalError(L"Parameter negative in multinomial function");
  ...
  for (i=0, sum=0; i<colors; i++) { 
    p = source[i];
//	if (p < 0) FatalError(L"Parameter negative in multinomial function");
    sum += p;}
//  if (sum == 0 && n > 0) FatalError(L"Zero sum in multinomial function");
...
*/
  int32 sum, x, y;
  int i;
//  if (n < 0 || colors < 0) FatalError(L"Parameter negative in multihypergeo function");
  if (colors == 0) return;
  
  // compute total number of balls
  for (i=0, sum=0; i<colors; i++) { 
    y = source[i];
//	if (y < 0) FatalError(L"Parameter negative in multihypergeo function");
    sum += y;}
//  if (n > sum) FatalError(L"n > sum in multihypergeo function");
